...
export default function UploadForm({
  ...
  const [sucesso, setSucesso] = useState<{ mandato: string; arquivos: number } | null>(null);
  ...
  async function enviar(e: React.FormEvent) {
    ...
    try {
      ...
      setSucesso({ mandato: json.mandato ?? mandato.trim(), arquivos: json.arquivos ?? arquivos.length });
      setArquivos([]);
    ...
    return (
      <div className="rounded border border-emerald-300 bg-emerald-50 p-4 text-sm text-emerald-900">
        ...
        <p className="mt-1 text-emerald-800">
          O N8N está processando (classificação + extração). Os documentos aparecem no mandato em
          instantes — atualize a página do caso para acompanhar.
        </p>
