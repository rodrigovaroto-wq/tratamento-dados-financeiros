# -*- coding: utf-8 -*-
"""Demonstrações e anexos do book ARAUCÁRIA, TODOS amarrados ao balanço.
...
# Receita bruta da Serraria por exercício (R$ mil). É a única série "de fora"
# do balanço, porque receita não é saldo — e é dela que o faturamento mensal, as
# deduções e o CMV descem.
#
# 2022 é o pico: madeira serrada em alta e exportação aquecida. A queda de 2023
# é preço (o pinus serrado cai 31%), não volume — e é essa distinção que o
# faturamento mensal em quantidade permite conferir.
RECEITA_BRUTA = {2021: 437_000, 2022: 412_000, 2023: 356_000, 2024: 298_000, 2025: 231_000}

# Câmbio de fechamento usado na tranche em dólar do mapa de dívida.
CAMBIO_FECHAMENTO = {2021: 5.5805, 2022: 5.2177, 2023: 4.8413, 2024: 6.1923, 2025: 5.4408}
...
def variacoes(bp, tot, ent="serraria"):
    ...
    # 2021 não tem exercício anterior no book: a movimentação é DECLARADA pela
    # nota explicativa, não medida. Fica explícito para ninguém confundir
    # "medido no balanço" com "afirmado pela empresa" — e o GUIA cobra essa
    # diferença, porque ela é a única linha da DRE que não pode ser conferida
    # contra o balanço.
    v[2021] = {"depreciacao": 6_100, "amort_du": 0, "amort_intangivel": 240,
               "pecld": 380, "obsolescencia": 0, "provisoes": 520, "ir_diferido": 0,
               "dea_total": 6_340}
    return v


def divida_bancaria(bp, ano, ent="serraria"):
...
# ---------------------------------------------------------------------------
# O primeiro exercício do book não tem anterior para variar contra: o resultado
# dele e os dividendos são DECLARADOS, não medidos. As duas únicas linhas do
# book nessa condição, e o GUIA as nomeia.
DIVIDENDOS_INICIAIS = 2_400
LUCRO_INICIAL = 26_900


def resultado_alvo(tot, ent="serraria"):
    """O que a DRE de cada exercício TEM de dar."""
    alvo = {ANOS[0]: LUCRO_INICIAL}
    for ano in ANOS[1:]:
        ant = ANOS[ANOS.index(ano) - 1]
...
def dre_industria(bp, tot):
    ...
    mix = {
        2021: dict(mp=.496, mod=.101, energia=.034, outros=.028, vendas=.053, ga=.042),
        2022: dict(mp=.512, mod=.104, energia=.036, outros=.029, vendas=.055, ga=.044),
    ...
    honorarios = {2021: 1_500, 2022: 1_600, 2023: 1_800, 2024: 1_900, 2025: 2_100}
    impairment = {2021: 0, 2022: 0, 2023: 0, 2024: -9_800, 2025: 0}
    reestruturacao = {2021: 0, 2022: 0, 2023: 0, 2024: 0, 2025: -4_200}
    rec_fin = {2021: 3_100, 2022: 2_400, 2023: 1_100, 2024: 420, 2025: 90}
    cambio = {2021: 2_600, 2022: 1_900, 2023: 380, 2024: -1_200, 2025: -2_100}
    encargo_transacao = {2021: 0, 2022: 0, 2023: 0, 2024: 0, 2025: -3_900}
    ir_corrente = {2021: -9_400, 2022: -7_800, 2023: -5_100, 2024: 0, 2025: 0}
    ...
    for ano in ANOS:
        ...
        linhas += [
            ...
            ("Vendas para empresas do grupo (Araucária Comercial)",
...
def juros_por_ano(bp):
    ...
    taxa = {2021: 0.129, 2022: 0.152, 2023: 0.181, 2024: 0.223, 2025: 0.268}  # custo médio efetivo crescente
    j = {}
    for ano in ANOS:
...
def faturamento_intragrupo(bp):
    ...
    linhas = []
    for ano in ANOS:
        agro = M.valor_conta(bp[ano]["florestal"], "AC", "Contas a Receber", "Contas a receber intragrupo")
        ind = M.valor_conta(bp[ano]["comercial"], "PC", "Fornecedores", "Fornecedores intragrupo")
        alug = M.valor_conta(bp[ano]["imob"], "AC", "Contas a Receber", "Aluguéis a receber - Araucária Serraria")
        frete = M.valor_conta(bp[ano]["transportes"], "AC", "Contas a Receber", "Conta corrente a receber")
        linhas.append((ano, [
            ("Araucária Florestal", "Araucária Serraria", "Madeira e resíduos", agro),
            ("Araucária Serraria", "Araucária Comercial", "Embalagens para revenda", ind),
            ("Araucária SPE", "Araucária Serraria", "Aluguel de galpões", alug),
            ("AR Transportes", "Araucária Serraria", "Fretes e logística", frete),
        ]))
    return linhas
...
def mutuos(bp, tot, ano=2025):
    saldos = []
    # `M.vivas` e não `D.CONTROLADAS`: a Ferragens foi incorporada em 2024, e
    # procurá-la em 2025 estoura. `M.mutuo_a_pagar` e não a leitura direta do
    # PC: metade do grupo classifica o mútuo no não circulante.
    for k in M.vivas(ano):
        v = M.mutuo_a_pagar(bp[ano][k])
        if v:
...
PERFIL_AR = {2021: [.83, .09, .04, .02, .01, .01],
             2022: [.79, .10, .05, .03, .02, .01],
             2023: [.72, .12, .07, .04, .03, .02],
...
CLIENTES = [
    ...
    "Cooperativa Agroindustrial Campos Gerais", "Demais clientes (312 sacados)",
]
...
def aging_recebiveis(bp, ano=2025):
    ...
    # Os DOIS nomes da mesma conta, porque a grafia muda em 2024 (troca de
    # escritório contábil — ver a armadilha `conta_renomeada_no_meio`). Casar só
    # com um deles devolve ZERO em metade da série, e zero aqui não estoura:
    # sai um aging de carteira vazia, que parece um documento legítimo.
    def cliente(*prefixos):
        for pref in prefixos:
            v = M.valor_conta(bp[ano]["serraria"], "AC", "Contas a Receber", pref)
            if v:
                return v
        return 0

    interno = cliente("Clientes mercado interno", "Duplicatas a receber - mercado interno")
    externo = cliente("Clientes mercado externo", "Duplicatas a receber - exportação")
    total = interno + externo
    assert total > 0, (
        f"aging AR {ano}: carteira ZERO. O rótulo da conta de clientes mudou e a busca "
        "não acompanhou — e um aging vazio sai como documento legítimo.")
...
def aging_pagaveis(bp, ano=2025):
    ...
    pc = bp[ano]["serraria"]["PC"]["Fornecedores"]
...
# ---------------------------------------------------------------------------
ITENS_ESTOQUE = [
    ("Estoques de madeira serrada", [
        ("Pinus serrado verde - 25 mm", "m³", 4_180),
        ("Pinus serrado seco em estufa - 25 mm", "m³", 2_940),
        ("Pinus serrado seco em estufa - 40 mm", "m³", 1_620),
        ("Eucalipto serrado - bitolas diversas", "m³", 980),
    ]),
    ("Madeira em toras - pátio", [
        ("Toras de pinus - diâmetro 18 a 25 cm", "m³", 9_400),
        ("Toras de pinus - diâmetro acima de 25 cm", "m³", 5_100),
        ("Toras de eucalipto", "m³", 2_260),
    ]),
    ("Produtos em elaboração", [
        ("Madeira em processo de secagem", "m³", 1_840),
        ("Peças em processo de beneficiamento", "m³", 720),
    ]),
    ("Materiais auxiliares e de manutenção", [
        ("Serras, facas e ferramentas de corte", "unidade", 2_180),
        ("Peças de reposição e rolamentos", "unidade", 4_600),
        ("Materiais de consumo geral", "unidade", 11_200),
    ]),
    ("Ferragens e componentes metálicos", [
        ("Dobradiças, corrediças e puxadores", "unidade", 84_000),
        ("Parafusos e fixadores", "milheiro", 3_400),
    ]),
]
...
def estoques(bp, ano=2025):
    ...
    est = {rot: v for rot, v in bp[ano]["serraria"]["AC"]["Estoques"]}
    grupos = []
    ...
    # Por PREFIXO, e não pelo rótulo exato: a grafia da provisão muda entre as
    # empresas e entre os exercícios deste book ("de estoques", "de coleção",
    # ou só "obsolescência"), e casar pelo texto inteiro devolveria ZERO —
    # o que faria o documento não somar o balanço sem dizer por quê.
    provisao = next((v for r, v in est.items()
                     if r.startswith("(-) Provisão para obsolescência")), 0)
    total = sum(g[2] for g in grupos) + provisao
    assert total == M.soma_folhas({"x": bp[ano]["serraria"]["AC"]["Estoques"]})
...
def situacao_fiscal(bp, ano=2025):
    ...
    parcelamentos = [
        {"entidade": "ARAUCÁRIA SERRARIA E BENEFICIAMENTO DE MADEIRAS LTDA.",
        ...
        {"entidade": "ARAUCÁRIA COMERCIAL E EXPORTAÇÃO LTDA.",
...
def contingencias(bp, ano=2025):
    ...
    prov = {r: v for r, v in bp[ano]["serraria"]["PNC"].get("Provisões", [])}
    provisionado = {
...
def imobilizado(bp, ano=2025):
    ...
    imo = {r: v for r, v in bp[ano]["serraria"]["ANC"]["Imobilizado"]}
...
def folha(ano=2025):
    ...
    R$ 148 mil."""
    linhas = []
    for area, hc_ini, hc_fim, medio in AREAS:
        # O quadro é interpolado linearmente entre o primeiro e o último
        # exercício do book. Interpolar, e não tabelar: com quatro exercícios a
        # tabela à mão é mais um lugar para o número divergir do resto, e a
        # demissão neste grupo foi contínua, não em degrau.
        passo = (hc_fim - hc_ini) / (len(ANOS) - 1)
        hc = int(round(hc_ini + passo * ANOS.index(ano)))
...
def extratos(bp, ano=2025):
    disp = M.soma_folhas({"x": bp[ano]["serraria"]["AC"]["Disponível"]})
    linhas, acum = [], 0
...
def dfc(bp, tot, ano=2025):
    ...
    def disp(a):
        return M.soma_folhas({"x": bp[a]["serraria"]["AC"]["Disponível"]})

    def bloco(a, secao, ignora=()):
        s = 0
        for sub, contas in bp[a]["serraria"][secao].items():
            if sub in ignora:
                continue
    ...
    op_passivo = (sum(sum(v for _, v in bp[ano]["serraria"]["PC"].get(s, [])) for s in op_passivo_subs)
                  - sum(sum(v for _, v in bp[ant]["serraria"]["PC"].get(s, [])) for s in op_passivo_subs))

    # Investimento: variação do custo do imobilizado e do intangível (bruto).
    def bruto(a, sub):
        return sum(v for r, v in bp[a]["serraria"]["ANC"].get(sub, []) if not r.startswith("(-)"))
    ...
    def fin(a):
        pc = sum(v for _, v in bp[a]["serraria"]["PC"].get("Empréstimos e Financiamentos", []))
        pnc = sum(v for _, v in bp[a]["serraria"]["PNC"].get("Empréstimos e Financiamentos", []))
        arr = (sum(v for _, v in bp[a]["serraria"]["PC"].get("Arrendamentos", []))
               + sum(v for _, v in bp[a]["serraria"]["PNC"].get("Arrendamentos", [])))
        mut = sum(v for _, v in bp[a]["serraria"]["PC"].get("Partes Relacionadas", []))
...
# ---------------------------------------------------------------------------
def dmpl(bp, tot):
    """A DMPL dos QUATRO exercícios, com as movimentações DERIVADAS do balanço.

    Por que derivada e não escrita à mão, como no `book-canastra`: aqui o
    período tem eventos societários de verdade — aumento de capital na
    incorporação da Ferragens (2024), realização da reserva de lucros a realizar
    (2023 e 2024) e absorção de prejuízo contra a reserva legal (2025). Escrever
    as linhas à mão significaria manter três tabelas em acordo com o plano de
    contas; derivá-las significa que uma mudança no plano OU aparece na DMPL, OU
    estoura o assert. Nunca passa calada.

    As linhas "SALDOS EM 31 DE DEZEMBRO DE ..." são SUBTOTAIS e não podem ser
    somadas ao PL — a armadilha herdada dos dois books anteriores, aqui
    ...
    def pl_conta(ano, prefixo):
        for sub in bp[ano]["serraria"]["PL"].values():
            for rot, v in sub:
    ...
    saldos = {a: saldo(a) for a in ANOS}

    # O saldo de ABERTURA do primeiro exercício é reconstruído, não medido: o
    # book começa em 2022 e não há 2021 para ler. Fica declarado no GUIA.
    ini_ = ANOS[0]
    abertura = list(saldos[ini_])
    abertura[3] = saldos[ini_][3] - LUCRO_INICIAL + DIVIDENDOS_INICIAIS
    abertura[4] = sum(abertura[:4])

    linhas = [(f"SALDOS EM 31 DE DEZEMBRO DE {ini_ - 1}", abertura, "saldo")]

    def movimentos(ant, atual, resultado_rotulo, resultado_valor):
        """As movimentações que levam `ant` a `atual`, cada uma nomeada."""
        d_cap = atual[0] - ant[0]
        d_leg = atual[1] - ant[1]
        d_rea = atual[2] - ant[2]
        out = []
        if d_cap:
            out.append(("Aumento de capital por incorporação de acervo líquido",
                        [d_cap, 0, 0, 0, d_cap], "mov"))
        if d_rea:
            out.append(("Realização da reserva de lucros a realizar",
                        [0, 0, d_rea, -d_rea, 0], "mov"))
        if d_leg:
            rot = ("Absorção de prejuízo com reserva legal" if d_leg < 0
                   else "Constituição de reserva legal")
            out.append((rot, [0, d_leg, 0, -d_leg, 0], "mov"))
        out.append((resultado_rotulo,
                    [0, 0, 0, resultado_valor, resultado_valor], "mov"))
        return out

    # Primeiro exercício: da abertura reconstruída ao saldo de 2022.
    linhas += movimentos(abertura, saldos[ini_],
                         f"Lucro líquido do exercício de {ini_}", LUCRO_INICIAL)
    linhas.append(("Dividendos deliberados e pagos",
                   [0, 0, 0, -DIVIDENDOS_INICIAIS, -DIVIDENDOS_INICIAIS], "mov"))
    linhas.append((f"SALDOS EM 31 DE DEZEMBRO DE {ini_}", saldos[ini_], "saldo"))

    for i in range(1, len(ANOS)):
        ant, ano = ANOS[i - 1], ANOS[i]
        # O RESULTADO é o resíduo: o que sobra em "acumulados" depois de tirar o
        # efeito das reservas. Assim ele não pode discordar do balanço, e a
        # variação do PL continua sendo a prova.
        d_acum = saldos[ano][3] - saldos[ant][3]
        d_leg = saldos[ano][1] - saldos[ant][1]
        d_rea = saldos[ano][2] - saldos[ant][2]
        resultado = d_acum + d_leg + d_rea
        rot = ("Lucro líquido" if resultado >= 0 else "Prejuízo") + f" do exercício de {ano}"
        linhas += movimentos(saldos[ant], saldos[ano], rot, resultado)
        linhas.append((f"SALDOS EM 31 DE DEZEMBRO DE {ano}", saldos[ano], "saldo"))

    # As amarrações: cada saldo bate com o balanço, e cada bloco de movimentação
    # leva de fato um saldo ao seguinte.
    for a in ANOS:
        assert saldos[a][4] == tot[a]["serraria"]["PL"], f"DMPL {a} não bate com o balanço"
    corrente = None
    for rotulo, valores, tipo in linhas:
        if tipo == "saldo":
            if corrente is not None:
                assert corrente == valores, (
                    f"a movimentação não leva ao saldo de «{rotulo}»: {corrente} != {valores}")
            corrente = list(valores)
        else:
            corrente = [corrente[k] + valores[k] for k in range(5)]
...
def razao_fornecedores(bp, ano=2025, lancamentos=96):
    ...
    saldo_final = M.valor_conta(bp[ano]["serraria"], "PC", "Fornecedores", "Fornecedores nacionais")
...
# ---------------------------------------------------------------------------
def balancete(bp, ano, ent="serraria"):
...
def dre_condensada(bp, tot, ent, receita, anos):
    ...
    blocos = {}
    for ano in anos:
        # Empresa que NASCEU neste exercício, ou primeiro ano do book: não há PL
        # anterior para variar contra. O resultado é o próprio PL — que é o que
        # acontece numa empresa constituída no ano.
        i = ANOS.index(ano)
        anterior_existe = i > 0 and ano - 1 in D.anos_da_entidade(ent)
        alvo = (tot[ano][ent]["PL"] - tot[ANOS[i - 1]][ent]["PL"]) if anterior_existe \
            else tot[ano][ent]["PL"]
        rb = receita[ano]
        ded = -round(rb * .148)
        rl = rb + ded
        ...
        div = divida_bancaria(bp, ano, ent)
        fin = -round(div * .24)
