# -*- coding: utf-8 -*-
"""Renderização do book ARAUCÁRIA em PDF.
...
DIRETOR = "Paulo Sérgio Wolff — Diretor Presidente — CPF 987.654.321-00"
...
# ---------------------------------------------------------------- COMBINADO --
def pdf_combinado(bp, tot, ano, arquivo, combinado=None, marca=False, titulo_extra=None):
    ...
    # `combinado` permite renderizar a versão PRELIMINAR (ver `bagunca.py`) sem
    # duplicar o layout: é o mesmo documento, com outro quadro de eliminações.
    c = combinado if combinado is not None else M.combinado(bp, tot, ano)
    ordem = c["ordem"]
    # O nome curto sai do próprio cadastro: com 14 empresas, um dicionário à
    # parte é mais um lugar para uma empresa nova entrar sem nome e sair como
    # chave técnica na coluna do documento.
    curtos = {k: v["nome_fantasia"] for k, v in D.ENTIDADES.items()}
    d = doc(arquivo, paisagem=True, marca=marca)
    el = cabecalho(D.GRUPO, None, "BALANÇO PATRIMONIAL COMBINADO",
                   f"Posição em 31 de dezembro de {ano}",
                   "(Valores expressos em milhares de reais — R$ mil)",
                   titulo_extra or "Elaborado para fins gerenciais — não auditado")
    ...
    el.append(Paragraph(
        ...
        "equivalência patrimonial. A participação de não controladores refere-se às "
        "participações de terceiros na AR Transportes Rodoviários Ltda. (30%) e na Araucária "
        "Varejo e Franquias Ltda. (45%).", ST_NOTA))
    _assina(el)
    build(d, el)
    return arquivo
...
def pdf_faturamento(arquivo):
    ...
    d = doc(arquivo)
    el = cabecalho(D.ENTIDADES["serraria"]["razao_social"], D.ENTIDADES["serraria"]["cnpj"],
                   f"RELATÓRIO DE FATURAMENTO — ÚLTIMOS {len(D.ANOS) * 12} MESES",
                   f"Janeiro de {D.ANOS[0]} a dezembro de {D.ANOS[-1]}",
...
def pdf_aging(titulo, entidade, faixas, linhas_aging, total, vencido, arquivo,
              rotulo_coluna, escala, formato="br", nota=None, ano=None):
    d = doc(arquivo, paisagem=True)
    el = cabecalho(entidade["razao_social"], entidade["cnpj"], titulo,
                   f"Posição em 31 de dezembro de {ano or D.ANOS[-1]}", escala)
...
def pdf_situacao_fiscal(bp, ano, arquivo):
    ...
    el.append(Spacer(1, 4 * mm))
    el.append(Paragraph("2. Tributos correntes a recolher (Araucária Serraria)", ST_H2))
...
def pdf_folha(ano, arquivo):
    ...
    _assina(el, contador="Departamento Pessoal — Araucária Serraria", diretor=None)
    build(d, el)
    return arquivo
...
# -------------------------------------------------------------- BALANCETE ---
def pdf_balancete(bp, ano, arquivo, ent="serraria"):
...
def pdf_certidoes(arquivo):
    ...
    dados = [
        ("Araucária Serraria e Beneficiamento de Madeiras Ltda.", "CND Federal (RFB/PGFN)", "Receita Federal",
         "Positiva com efeito de negativa", "11/07/2026"),
        ("Araucária Serraria e Beneficiamento de Madeiras Ltda.", "CRF - FGTS", "Caixa Econômica Federal",
         "Regular", "20/02/2026"),
        ("Araucária Serraria e Beneficiamento de Madeiras Ltda.", "CNDT - Débitos Trabalhistas", "TST",
         "Positiva", "-"),
        ("Araucária Serraria e Beneficiamento de Madeiras Ltda.", "Certidão Estadual de ICMS", "SEFAZ/PR",
         "Positiva com efeito de negativa", "08/04/2026"),
        ("Araucária Comercial e Exportação Ltda.", "CND Federal (RFB/PGFN)", "Receita Federal",
         "Positiva", "-"),
        ("Araucária Comercial e Exportação Ltda.", "Certidão Estadual de ICMS", "SEFAZ/PR",
         "Positiva", "-"),
        ("AR Transportes e Logística Ltda.", "CND Federal (RFB/PGFN)", "Receita Federal",
         "Negativa", "03/06/2026"),
        ("Araucária Florestal e Reflorestamento Ltda.", "CND Federal (RFB/PGFN)", "Receita Federal",
         "Negativa", "27/05/2026"),
        ("Araucária Imobiliária SPE Ltda.", "Certidão de Débitos Municipais", "Prefeitura",
         "Negativa", "14/03/2026"),
    ]
    for row in dados:
...
def pdf_contrato_social(bp, tot, arquivo):
    capital = 0
    for sub in bp[2025]["serraria"]["PL"].values():
        for rot, v in sub:
    ...
    for titulo, texto in [
        ("CLÁUSULA PRIMEIRA — DENOMINAÇÃO E SEDE",
         "A sociedade gira sob a denominação de ARAUCÁRIA INDÚSTRIA DE EMBALAGENS LTDA., com sede "
        ...
        ("CLÁUSULA TERCEIRA — CAPITAL SOCIAL",
         ...
         "subscrito e integralizado em moeda corrente nacional, assim distribuído: ARAUCÁRIA "
         "PARTICIPAÇÕES S.A., detentora de 100% das quotas."),
        ("CLÁUSULA QUARTA — ADMINISTRAÇÃO",
         "A administração da sociedade compete ao sócio administrador Paulo Sérgio Wolff, "
    ...
    _assina(el, contador="Paulo Sérgio Wolff — Sócio Administrador",
...
def pdf_organograma(arquivo):
    """As 14 empresas, com a VIGÊNCIA de cada uma.

    A coluna "Situação" é o que impede a leitura ingênua do kit: três empresas
    não existiram nos quatro exercícios, e sem essa coluna o organograma afirma
    um grupo de 13 empresas simultâneas que nunca existiu."""
    d = doc(arquivo)
    el = cabecalho(D.GRUPO, None, "ORGANOGRAMA SOCIETÁRIO",
                   f"Posição em 31 de dezembro de {D.ANOS[-1]}, com o histórico do período",
                   None)
    linhas = [["Controlada", "CNPJ", "Part.", "Atividade", "Situação no período"]]
    for k in D.CONTROLADAS:
        e = D.ENTIDADES[k]
        anos = D.anos_da_entidade(k)
        situacao = ("Em atividade nos quatro exercícios" if len(anos) == len(D.ANOS)
                    else e.get("evento", f"Exercícios de {anos[0]} a {anos[-1]}"))
        linhas.append([
            Paragraph(e["razao_social"], ST_CEL), e["cnpj"],
            pct(D.PARTICIPACAO[k] * 100, 0),
            Paragraph(e["atividade"], ST_CEL), Paragraph(situacao, ST_CEL),
        ])
    el.append(_tab(linhas, [44 * mm, 26 * mm, 13 * mm, 44 * mm, 55 * mm], fonte=6.6))
    el.append(Spacer(1, 4 * mm))
    el.append(Paragraph(
        "A controladora de todas as sociedades acima é a ARAUCÁRIA PARTICIPAÇÕES E "
        "EMPREENDIMENTOS S.A. (CNPJ 27.884.115/0001-08), cujo quadro societário é composto por "
        "quatro pessoas físicas da mesma família. Os 30% restantes do capital da AR Transportes "
        "Rodoviários Ltda. e os 45% restantes do capital da Araucária Varejo e Franquias Ltda. "
        "pertencem a sócios sem vínculo com o grupo controlador.", ST_TXT))
    el.append(Spacer(1, 3 * mm))
    el.append(Paragraph(
        "A Araucária Florestal e Reflorestamento Ltda. teve sua razão social alterada em 2023; "
        "documentos emitidos antes dessa data, e alguns emitidos depois, referem-se a ela como "
        "«Campos Gerais Reflorestamento Ltda.». Ela NÃO se confunde com a Campos Gerais "
        "Agropecuária Ltda. (CNPJ 27.884.124/0001-80), sociedade distinta do mesmo grupo.", ST_TXT))
    el.append(Spacer(1, 3 * mm))
    el.append(Paragraph(
...
def pdf_notas(bp, tot, arquivo):
    ...
    t25 = tot[2025]["serraria"]
    t24 = tot[2024]["serraria"]
    ccl = t25["AC"] - t25["PC"]
    divida = X.divida_bancaria(bp, 2025)
    for titulo, texto in [
        ("1. CONTEXTO OPERACIONAL E CONTINUIDADE",
         "A Araucária Serraria e Beneficiamento de Madeiras Ltda. e suas partes relacionadas atuam na fabricação "
        ...
        ("5. PARTES RELACIONADAS",
         ...
         "fornecimento de matéria-prima pela Araucária Florestal e Reflorestamento Ltda., locação de galpões da "
         "Araucária Imobiliária SPE Ltda. e serviços de transporte prestados pela AR Transportes e "
...
def pdf_parecer(arquivo):
    ...
    for titulo, texto in [
        ("OPINIÃO COM RESSALVA",
         "Examinamos as demonstrações contábeis da ARAUCÁRIA INDÚSTRIA DE EMBALAGENS LTDA., que "
...
def pdf_df_completa(bp, tot, ano, arquivo):
    ...
    el.append(tabela_balanco(bp, tot, "serraria", [ano, ant]))
    el.append(PageBreak())
