...
- **O SCHEMA é reproduzível do zero.** `db/migrations/` aplicadas em ordem reconstroem o banco
  inteiro, e o `db/test/run.sh` faz exatamente isso a cada execução, contra Postgres 16. O
  `db/schema.sql` é o espelho materializado, conferido no CI.
