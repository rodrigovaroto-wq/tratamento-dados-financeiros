...
quatro suítes rodadas localmente, dos artefatos de F0 (`f0/04`, `f0/07`, `f0/08`), dos `docs/`, do
...
3. **Nenhum número de produção governa nada.** Não há golden set, não há concordância medida, não há
   ...
   tempo levaram. O dial de autonomia está onde nasceu na F1, em 2026-07. `docs/03` chama F4 de
...
outra decisão. Ou se aceita explicitamente (e se registra no `f0/07`, que promete proveniência por
...
`f0/07` define **dois modos de entrega**. O Modo B (Excel) está entregue e maduro. O **Modo A — base
...
"base viva" que `f0/07` chama de entrega principal é o Excel.

Isso é uma escolha legítima — mas está tomada por omissão, não por decisão. Vale escrever qual das
duas é a verdade: ou o Modo A entra no roadmap, ou `f0/07` passa a dizer que o Excel é a entrega e o
portal é a esteira.

### 2.5 O que a `f0/08` prometeu fasear continua faseado (MÉDIO, MÉDIO)

O faseamento honesto de `f0/08` lista o que fica de fora "até a extração isolar as linhas-conceito":
...
DRE tem despesa financeira. O bloqueio documentado em `f0/08` **não vale mais para o arquivo de
...
também rode `pg_dump --schema-only` e versione o resultado em `db/schema.sql`, com
...
"não aplicada" têm exatamente a mesma aparência — é a frase do próprio `db/README.md`.
...
- `f0/04` exige **papel sênior** para aceitar com ressalva → a ressalva **não pode ser
...
`f0/04` define seis estados. `aberta` (motor) e `resolvida` (`sistema:*`) sempre existiram;
...
2. **Não existe golden set.** `docs/01` exige concordância medida no estrato que vai a produção
...
| **Migration aplicada à mão** | `db/README.md`, lista de comandos | ver 3.2 — já custou uma rodada |
...
| 2 | **`db/schema.sql` gerado pelo CI** + `schema_migrations` com aviso no portal | CI, `db/` | 🔴 alto (para de repetir a `0101`) | baixo |
| 3 | **Índices que a `f0/08` faseou e o modelo já isola** — cobertura de juros, ciclo de caixa, liquidez seca | `Output`/`Balance Sheet` | 🟠 médio-alto | baixo |
| 4 | **Fechar o PR #112 com a fixture de extração** do `book-canastra` | `test-data/`, `db/test/` | 🔴 alto (é a única prova sobre dado sujo) | médio |
...
| 7 | **Papel de usuário** (analista/sênior) → destrava a ressalva de `f0/04` e limita quem rejeita | `db/`, portal | 🟠 médio | médio |
| 8 | **`reenviada_ao_cliente` e `em_correcao_interna`** com ação de tela | portal, `db/` | 🟠 médio | médio |
...
| 13 | **Modo A de `f0/07`** — base viva consultável — ou a decisão escrita de que ele não vem | portal | 🟠 médio | alto |
