...
function garantirMolde() {
  ...
  const arquivos = execFileSync("bash", ["-c", "ls db/migrations/*.sql"], { encoding: "utf8", cwd: RAIZ })
    .trim().split("\n");
...
async function rodar(v: Variacao) {
  ...
  for (const doc of fixture.documentos) {
    ...
    mkdirSync(`${RAIZ}test/e2e/saida`, { recursive: true });
    const tmp = `${RAIZ}test/e2e/saida/_campos-${v.nome}.sql`;
  ...
  if (wb) {
    ...
    mkdirSync(`${RAIZ}test/e2e/saida`, { recursive: true });
    await wb.xlsx.writeFile(`${RAIZ}test/e2e/saida/modelagem-${v.nome}.xlsx`);
...
mkdirSync(`${RAIZ}test/e2e/saida`, { recursive: true });
writeFileSync(`${RAIZ}test/e2e/saida/variacoes.json`, JSON.stringify({ resumo, achados }, null, 2));
console.log(`\n${achados.length} achado(s) no total. Detalhe em test/e2e/saida/variacoes.json`);
