...
const alvos = extraido.nosPostgres
  .map(({ arquivo, no, consulta }) => `    [${lit(`${arquivo} · ${no}`)}, ${lit(consulta)}]`)
  .join(",\n");

process.stdout.write(CABECALHO + `create temp table if not exists _conferir(no text, chamada text, resolve boolean, erro text);
truncate _conferir;
do $conf$
declare
  r record; i int := 0;
