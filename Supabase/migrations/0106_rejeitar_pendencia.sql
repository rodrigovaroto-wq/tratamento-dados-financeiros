...
--
-- `Arquitetura do Sistema/2 Especificação/f0/04` define a máquina de estado da pendência com TRÊS saídas:
--
...
-- não é preguiça: `Arquitetura do Sistema/2 Especificação/f0/04` exige papel **sênior** para ressalvar, e papel de
...
--
-- Isso não é defeito da `0037`: é o que `Arquitetura do Sistema/2 Especificação/f0/04` manda ("tem de ser resolvida ou
...
begin
  ...
  -- condições — rejeitada é terminal, e é assim que `Arquitetura do Sistema/2 Especificação/f0/04` a define. Entra no
...
begin
  ...
  -- o que `override` quer dizer em `Arquitetura do Sistema/2 Especificação/f0/05`. Não é `ressalva` — ressalva admite o
...
comment on function fn_rejeitar_pendencia(uuid, text, text) is
  'Declara uma pendência IMPROCEDENTE (Arquitetura do Sistema/2 Especificação/f0/04: estado `rejeitada`). Exige motivo com no mínimo '
