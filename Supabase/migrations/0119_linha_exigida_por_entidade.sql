...
--
-- NUMERAÇÃO: esta migration nasceu como `0116` no PR #135 e foi renumerada para
-- `0119` porque a faixa 0116-0118 foi ocupada pelo PR #136, mergeado antes. Não
-- é detalhe de arrumação: o portão do `Supabase/test/run.sh` REPROVA prefixo
-- duplicado, e com razão — duas migrations com o mesmo número entram por
-- branches diferentes sem uma palavra do git, e a ordem oficial de aplicação
-- do `Supabase/README.md` passa a mentir em silêncio. O conteúdo é o do PR #135.
--
...
-- Supabase/test/linha_exigida_entidade.test.sql.
-- -----------------------------------------------------------------------------
do $$
declare
  ...
  v_ent uuid;
begin
  -- A ALAVANCA NASCE NULL, E ESTA VERIFICAÇÃO NÃO PODE PUNIR QUEM A USOU.
  --
  -- A forma original reprovava com `raise exception` se ENCONTRASSE qualquer
  -- `escopo_entidade` não nulo. A intenção é certa — provar que a migration não
  -- define política —, mas o efeito era um alçapão: no dia em que o dono
  -- ligasse o override do COMBINADO (que é exatamente o que o cabeçalho manda
  -- ele fazer) e depois reaplicasse a lista de comandos do `Supabase/README.md` —
  -- que é como este projeto aplica migration —, a migration MORRERIA no meio,
  -- por causa de uma decisão legítima dele.
  --
  -- O que se quer provar é sobre a MIGRATION, não sobre o banco: que a coluna
  -- não tem DEFAULT e que este arquivo não escreve política nenhuma. É isso que
  -- se verifica agora. Override existente vira NOTICE — informação, não bloqueio.
  select count(*) into v_n
  from pg_attrdef ad
  join pg_attribute a on a.attrelid = ad.adrelid and a.attnum = ad.adnum
  where ad.adrelid = 'taxonomia_linha_exigida'::regclass and a.attname = 'escopo_entidade';
  if v_n <> 0 then
    raise exception '0119: escopo_entidade ganhou DEFAULT — a alavanca é do dono e tem de nascer NULL';
  end if;

  select count(*) into v_n from taxonomia_linha_exigida where escopo_entidade is not null;
  if v_n <> 0 then
    raise notice '0119: % exigência(s) já têm escopo_entidade definido — override do dono, preservado', v_n;
  end if;
...
-- =============================================================================
-- A VARREDURA DOS CASOS EXISTENTES NÃO MORA AQUI — e a separação é deliberada.
--
-- O PR #135 terminava com um bloco que chamava `fn_recomputar_completude` para
-- CADA caso já gravado, para as pendências de formato velho migrarem na hora.
-- A intenção é boa (é a doutrina da 0043: reaplicar as regras de hoje sobre o
-- dado que já está no banco), mas o lugar estava errado, por duas razões:
--
--   1. É a única parte que TOCA caso existente. Uma migration que muda
--      estrutura e função é revisável lendo o arquivo; uma que também reescreve
--      a fila de trabalho de todo mandato do escritório é outra categoria de
--      risco, e misturar as duas tira do dono a chance de dizer "aplique a
--      estrutura hoje, a varredura amanhã".
--   2. O efeito visível não é neutro: numa base com muitos mandatos, a
--      varredura faz aparecer uma LEVA de pendências novas de uma vez, sem
--      ninguém ter enviado nada — e a fila do painel é a tela em que se confia
--      para saber o que precisa de atenção HOJE.
--
-- A varredura continua existindo, como script à parte:
-- `Supabase/varredura_linha_exigida.sql`. Ela é idempotente (recomputar é o que o
-- pipeline já faz a cada extração), aceita rodar num caso só, e o
-- `Supabase/README.md` a lista como passo OPCIONAL, depois da migration. Sem ela,
-- nada fica órfão: cada caso migra de formato no próximo recomputo natural.
-- =============================================================================
