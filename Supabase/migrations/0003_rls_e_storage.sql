...
--
-- Contexto de segurança (Arquitetura do Sistema/1 Visão e Doutrina/02 §"três travas de stack", Arquitetura do Sistema/2 Especificação/f0/05 política LGPD):
...
-- Autenticado pode inserir e ler; nunca update/delete (imutável — Arquitetura do Sistema/1 Visão e Doutrina/01 nº 7).
...
-- extra para documentos PII sensível (só papéis autorizados) — Arquitetura do Sistema/2 Especificação/f0/05.
...
-- Acesso ao bucket privado só para autenticados (nunca URL pública — Arquitetura do Sistema/2 Especificação/f0/05).
