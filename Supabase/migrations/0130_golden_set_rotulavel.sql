...
-- cinco métricas do `Arquitetura do Sistema/2 Especificação/f0/06`, o portão `fn_golden_suficiente` e a regra de ouro
...
--
-- Fechamento #5 do `Arquitetura do Sistema/1 Visão e Doutrina/01` (anti-ancoragem): nenhum número da máquina vira
...
-- opcional por conveniência: o `Arquitetura do Sistema/2 Especificação/f0/06` amarra o rótulo à versão da taxonomia em
...
comment on function fn_golden_abrir_rodada(text, text, text, int) is
  'Abre uma rodada de calibração do Arquitetura do Sistema/2 Especificação/f0/06. Recusa nome repetido em vez de deixar o unique estourar: '
...
--
-- Devolve o que a tela precisa para MONTAR a amostra estratificada do `Arquitetura do Sistema/2 Especificação/f0/06`, e
...
comment on function fn_golden_candidatos(uuid) is
  ...
  'já-rotulado. Não sorteia a amostra de propósito: a estratificação do Arquitetura do Sistema/2 Especificação/f0/06 é escolha humana, e '
...
comment on function fn_golden_linhas_para_rotular(uuid, uuid, text) is
  'As rubricas que a extração achou, SEM O VALOR. Rotulagem cega (fechamento #5 do Arquitetura do Sistema/1 Visão e Doutrina/01): quem vê '
...
comment on function fn_golden_rotular(uuid, uuid, text, text, text, text, boolean, legibilidade, text, text) is
  'Grava o julgamento humano do documento (Arquitetura do Sistema/2 Especificação/f0/06, "o que é rotulado"), um por rotulador. Recusa tipo '
...
comment on function fn_golden_progresso(uuid) is
  'Quanto falta para a rodada bater o N do Arquitetura do Sistema/2 Especificação/f0/06, por tipo. Traz DUAS contagens de propósito: '
