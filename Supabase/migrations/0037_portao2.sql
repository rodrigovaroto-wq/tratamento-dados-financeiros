...
-- do Portão 2 está ESPECIFICADA E APROVADA em `Arquitetura do Sistema/2 Especificação/f0/04` desde a F0 ("Teto de
-- ressalvas confirmado em 3", lista fechada de não-sobrepujáveis, três condições
-- determinísticas) e `Arquitetura do Sistema/1 Visão e Doutrina/03` a põe dentro do MVP — mas nada disso é código.
--
...
--
-- Esta migration implementa a regra COMO ELA ESTÁ ESCRITA em `Arquitetura do Sistema/2 Especificação/f0/04`, sem
...
-- ---------------------------------------------------------------------------
-- `Arquitetura do Sistema/2 Especificação/f0/04` diz: "Ao expirar, a pendência REABRE automaticamente e o caso pode
...
comment on function fn_teto_ressalvas() is
  ...
  'Arquitetura do Sistema/2 Especificação/f0/04 ("Teto de ressalvas confirmado em 3") — não é palpite deste código.';

grant execute on function fn_teto_ressalvas() to authenticated;

-- -----------------------------------------------------------------------------
-- fn_avaliar_portao2 — a regra de `Arquitetura do Sistema/2 Especificação/f0/04`, determinística e sem efeito colateral.
--
...
begin
  ...
  -- em tratamento (correção interna / reenviada ao cliente) — é o que `Arquitetura do Sistema/2 Especificação/f0/04`
  ...
  -- LIBERAVA. Isto é exatamente a porta dos fundos que a lista fechada de Arquitetura do Sistema/2 Especificação/f0/04
  ...
  if v_nao_sobrepujavel > 0 then
    v_motivos := array_append(v_motivos,
      format('%s pendência(s) NÃO-SOBREPUJÁVEL(is) viva(s): nenhuma ressalva libera (lista '
             'fechada de Arquitetura do Sistema/2 Especificação/f0/04) — tem de ser resolvida ou rejeitada', v_nao_sobrepujavel));
  end if;
  ...
  if v_ressalvas_venc > 0 then
    v_motivos := array_append(v_motivos,
      format('%s ressalva(s) EXPIRADA(s): voltam a valer como pendência (Arquitetura do Sistema/2 Especificação/f0/04)',
             v_ressalvas_venc));
  end if;
...
comment on function fn_avaliar_portao2(uuid) is
  'Regra do Portão 2 de Arquitetura do Sistema/2 Especificação/f0/04, determinística e sem efeito colateral: elegível se e somente se '
...
comment on function fn_aprovar_caso(uuid, text, text) is
  ...
  'para nenhum autor: a regra de Arquitetura do Sistema/2 Especificação/f0/04 é determinística.';
