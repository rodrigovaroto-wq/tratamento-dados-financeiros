...
--
-- 2. DOUTRINA. `Arquitetura do Sistema/3 Estado e Execução/07` separa os dois status de propósito: "Completude ≠
...
--     é o que de fato trava o Portão 2. `Arquitetura do Sistema/3 Estado e Execução/07` lista "arquivo ilegível de
...
begin
  ...
  foreach v_cod in array v_sem_conteudo loop
    if not exists (
      ...
      -- BLOQUEANTE e NÃO-SOBREPUJÁVEL (sobrepujavel=false), sempre: `Arquitetura do Sistema/3 Estado e Execução/07`
  ...
  -- `recebido_nao_valido` é o vocabulário de `Arquitetura do Sistema/3 Estado e Execução/07` ("recebido_não_válido").
  ...
  -- acima — é assim que `Arquitetura do Sistema/3 Estado e Execução/07` desenha o portão ("elegível ao Portão 2 se e
