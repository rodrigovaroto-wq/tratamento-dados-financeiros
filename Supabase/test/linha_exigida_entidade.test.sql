-- Testes de "linha exigida POR ENTIDADE" (Supabase/migrations/0119).
...
begin
  ...
  -- ---------------------------------------------------------------------------
  -- 8. AS DUAS CORREÇÕES DA INCORPORAÇÃO (a 0119 é a 0116 do PR #135, revisada)
  -- ---------------------------------------------------------------------------
  raise notice '--- 8. o override do dono não pode virar alçapão na REAPLICAÇÃO ---';
  -- O QUE ACONTECIA. A verificação embutida da versão original reprovava com
  -- `raise exception` se encontrasse QUALQUER `escopo_entidade` não nulo. A
  -- intenção era certa (provar que a migration não define política), mas o
  -- efeito era um alçapão: no dia em que o dono ligasse o override do COMBINADO
  -- — que é exatamente o que o cabeçalho manda ele fazer — e depois reaplicasse
  -- a lista de comandos do `Supabase/README.md`, a migration MORRERIA no meio por
  -- causa de uma decisão legítima dele. Medido: a versão original aborta com
  -- "3 exigência(s) com escopo_entidade definido no seed".
  --
  -- O que se prova agora é sobre a MIGRATION, não sobre o banco: a coluna não
  -- pode ter DEFAULT. Override existente vira NOTICE, não exceção.
  select count(*) into v_n
  from pg_attrdef ad
  join pg_attribute a on a.attrelid = ad.adrelid and a.attnum = ad.adnum
  where ad.adrelid = 'taxonomia_linha_exigida'::regclass and a.attname = 'escopo_entidade';
  perform teste_assert_lee(v_n = 0,
    'escopo_entidade não tem DEFAULT — a alavanca nasce NULL porque a coluna não opina',
    v_n || ' default(s)');

  raise notice '--- 9. o filtro caro roda uma vez por TIPO, não por documento ---';
  -- POR QUE ISTO É UM TESTE DE TEXTO, e não de relógio. A propriedade real é
  -- "fn_linhas_do_tipo é chamada uma vez por tipo"; medi-la por tempo daria um
  -- teste que falha em máquina lenta e passa com o defeito de volta em máquina
  -- rápida. O que a sustenta é UMA palavra no SQL: sem `as materialized`, o
  -- Postgres achata a CTE e empurra o filtro de volta para baixo do
  -- agrupamento — foi o que o `explain analyze` mostrou acontecendo, e por isso
  -- a primeira tentativa de conserto não mudou nada no relógio.
  --
  -- Medido num caso sintético de 400 documentos e 16 mil linhas:
  --   • forma do PR #135 ....... 693 ms
  --   • com `as materialized` ... 116 ms   (5,9× mais rápido)
  select regexp_replace(lower(p.prosrc), '\s', '', 'g') into v_txt
  from pg_proc p where p.proname = 'fn_exigencias_do_caso';
  perform teste_assert_lee(position('asmaterialized' in v_txt) > 0,
