-- Testes do dial de autonomia (Supabase/migrations/0041).
...
begin
  ...
  perform teste_assert_dial(
    ...
    'a subida ficou registrada em evento_auditoria (Arquitetura do Sistema/1 Visão e Doutrina/01: versionada e reversível)');
  ...
  select count(*) into v_n from campo_extraido
    where documento_versao_id = v_ver and status_aceite = 'aceito';
  perform teste_assert_dial(v_n = 0,
    'N1 é sugestão (Arquitetura do Sistema/1 Visão e Doutrina/01): a linha entra pendente para o humano decidir',
    format('aceitas indevidamente: %s', v_n));
  ...
  -- Arquitetura do Sistema/1 Visão e Doutrina/01 cobra medição ou motivo assumido.
  --
  ...
  -- Arquitetura do Sistema/1 Visão e Doutrina/01: o teto é por natureza do estágio e nenhuma chamada o sobrepõe.
  ...
  raise notice '--- 8. o teto de N1 é respeitado nos estágios que Arquitetura do Sistema/1 Visão e Doutrina/01 nunca solta ---';
