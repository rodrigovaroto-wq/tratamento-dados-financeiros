...
try {
  arquivosDeWorkflow = readdirSync(join(RAIZ, "N8N")).filter((f) => /^workflow.*\.json$/.test(f));
} catch (erro) {
  desistir(`NÃO CONSEGUI LER N8N/ — ${erro.message}`, "Sem os workflows não há o que conferir.");
}
...
for (const arq of arquivosDeWorkflow) {
  ...
  try {
    ...
    desistir(`N8N/${arq} NÃO É JSON VÁLIDO — ${erro.message}`);
  }
  for (const no of wf.nodes ?? []) {
    ...
    nosPostgres.push({ origem, consulta });
  }
}
