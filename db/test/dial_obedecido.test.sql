-- Testes do dial OBEDECIDO (db/migrations/0127).
...
--      docs/01, e a Classe B/C declarava N0 abrindo pendência desde a 0015.
...
begin
  ...
  perform teste_assert_do((v_r->>'no_teto')::boolean,
    'e isso a coloca no TETO dela, que o docs/01 marca como nunca autônoma', v_r::text);
  ...
  v_r := fn_mudar_dial('classificacao_doc_checklist', 'N1', 'teste:dial-obedecido',
                       'N1 = humano confirma todo item (docs/01)');
  ...
  perform fn_mudar_dial('reconciliacao_classe_bc', 'N0', 'teste:dial-obedecido',
                        'sombra: registra e não influencia (docs/01)');
  v_r := fn_registrar_reconciliacao(
