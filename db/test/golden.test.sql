-- Testes do golden set e do portão da regra de ouro (db/migrations/0126).
-- Rodar via db/test/run.sh (que aplica as migrations antes).
--
-- O QUE ESTES TESTES TRAVAM. Antes da 0126, `docs/01` fechava com uma regra de
...
--   4. ONDE OS HUMANOS DISCORDAM, A MÁQUINA NÃO É COBRADA. `f0/06`: "se humanos
...
--
-- POR QUE OS CENÁRIOS USAM 20+ DOCUMENTOS E NÃO 2. O `n_minimo` do `f0/06` é ~20
...
begin
  ...
  perform teste_assert_golden(v_r->>'motivo_recusa' like '%concordância medida%',
    'e a recusa cita a exigência do docs/01, não um erro genérico', v_r->>'motivo_recusa');
  ...
  perform teste_assert_golden(
    ...
    'a tentativa recusada fica na trilha — é o que ela precisa guardar (docs/01)');
  ...
  exception when check_violation then
    perform teste_assert_golden(true,
      'e a rodada não DESCONGELA — ampliar é rodada nova (f0/06)');
  end;
  ...
  perform teste_assert_golden((v_med->>'aplica')::boolean = false
    ...
    'estágio determinístico não é cobrado pela regra de ouro (docs/01)', v_med::text);
  ...
  -- primeiro. A ordem das guardas importa e é esta que docs/01 exige.
