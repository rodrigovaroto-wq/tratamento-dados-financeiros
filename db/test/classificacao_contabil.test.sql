-- Testes da classificação contábil em sombra (db/migrations/0128).
...
-- `docs/03`, `docs/05` e da semeadura do dial na `0002` até aqui sem uma linha de
...
--      `classe_efetiva` = só o override humano. É o fechamento #5 do `docs/01`
...
begin
  ...
  perform teste_assert_cc(length(coalesce(v_r->>'justificativa','')) > 30,
    'e vem com justificativa escrita — o docs/05 exige em toda sugestão');
  ...
  perform teste_assert_cc(v_r->'classe_efetiva' = 'null'::jsonb,
    ...
    'este é o assert mais importante do arquivo: fechamento #5 do docs/01');
  ...
  perform teste_assert_cc((v_r->>'recusado')::boolean,
    'rótulo fora do catálogo é RECUSADO — a taxonomia do docs/05 é fechada', v_r::text);
  ...
  perform teste_assert_cc((v_r->>'recusado')::boolean
    ...
    'e ela nunca vira autônoma: o teto N1 recusa, como o docs/01 exige "para sempre"',
    v_r::text);
