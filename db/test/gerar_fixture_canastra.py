...
Uso (de dentro de test-data/book-canastra, que é onde os módulos do book vivem):

    cd test-data/book-canastra
    python3 ../../db/test/gerar_fixture_canastra.py > ../../db/test/fixture_book_canastra.sql

O `.sql` gerado é versionado para que `db/test/run.sh` rode sem Python — igual ao
...
# taxonomia (`db/migrations/0002`) não tem tipo para anexo de composição de
...
# ---------------------------------------------------------------------- emitir
out = [
    "-- GERADO por db/test/gerar_fixture_canastra.py — não editar à mão.",
    "-- Extração FIEL dos documentos de test-data/book-canastra (o book DIFÍCIL).",
