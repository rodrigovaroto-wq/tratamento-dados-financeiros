...
--       foi revisado — e o `db/test/run.sh` reprova quando ela fica para trás da
...
-- O `db/test/run.sh` lê esta linha do banco que ele acabou de montar e a compara
...
comment on table instalacao_cobertura is
  'Até que migration o catálogo instalacao_requisito foi revisado. Uma linha só. O db/test/run.sh '
