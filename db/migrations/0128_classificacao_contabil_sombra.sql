...
--
-- O `docs/03` lista oito estágios "dentro do MVP (tudo isto existe na v1 de
-- produção)", e um deles é *"Classificação contábil — presente em N0 (sombra):
-- registra sugestão, não decide"*. O `docs/05` o especifica por inteiro: taxonomia
...
-- segue a condição 2 do próprio `docs/05` — *"bate com um padrão conhecido
...
-- default conservador que o `docs/05` manda ser o default.
--
...
--    sexto rótulo para isso: o dono fechou a taxonomia nos cinco do `docs/05`, e
...
--
-- 2. A TAXONOMIA É TABELA, NÃO ENUM. Os cinco rótulos são os do `docs/05`, sem
...
--
-- 3. O AUTO-ACEITE DO `docs/05` NÃO É IMPLEMENTADO, E O MOTIVO É UMA CONTRADIÇÃO
--    DO PRÓPRIO DOCUMENTO. O `docs/05` tem uma seção inteira sobre "quando a
...
--    fica nomeada aqui e no `docs/05`.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- classe_contabil_catalogo — os cinco rótulos do `docs/05`, versionados.
-- -----------------------------------------------------------------------------
...
comment on table classe_contabil_catalogo is
  'A taxonomia contábil fechada do docs/05 (recorrente, nao_recorrente, extraordinario, '
...
insert into classe_contabil_catalogo (codigo, nome, descricao, ordem) values
  ...
  ('revisar_manual', 'Revisar manualmente',
   'O DEFAULT DE ESCAPE do docs/05. Rubrica que o catálogo não conhece, ou conhece e não determina '
   'recorrência. Não é falha da regra: é a regra dizendo que não sabe.', 5)
on conflict (codigo) do nothing;

-- -----------------------------------------------------------------------------
-- rubrica_classe — o "padrão conhecido pré-registrado" do `docs/05`, condição 2.
--
...
--
-- Cada linha carrega `justificativa` NOT NULL, e isso não é burocracia: o `docs/05`
...
comment on table rubrica_classe is
  'Mapeamento rubrica -> classe contábil: a "condição 2" do docs/05 ("bate com um padrão conhecido '
  'pré-registrado"). Semeado SÓ com rubricas medidas nos dois books e no caso de referência — '
  'rubrica imaginada criaria uma segunda régua. justificativa é NOT NULL porque o docs/05 exige '
...
comment on table campo_classe_sugerida is
  'A sugestão da classificação contábil, com a justificativa que o docs/05 exige de toda sugestão. '
...
-- -----------------------------------------------------------------------------
-- campo_classe_override — o "registro de override humano" do `docs/05`.
--
...
comment on table campo_classe_override is
  'A decisão humana sobre a classe contábil de uma linha (docs/05, "registro de override humano"). '
...
-- -----------------------------------------------------------------------------
-- fn_classe_contabil_sugerir — a regra determinística, condição 2 do `docs/05`.
--
...
comment on function fn_classe_contabil_sugerir(text, text, text) is
  'A regra determinística do docs/05 (condição 2: casa com padrão pré-registrado). NULL = a pergunta '
...
-- override**. A sugestão nunca é fato: é o fechamento #5 do `docs/01` ("nenhum
...
comment on function fn_classe_contabil_do_campo(uuid) is
  ...
  'objeto, separada, e nunca conta como fato: é o fechamento #5 do docs/01 aplicado à '
...
comment on function fn_registrar_classe_override(uuid, text, text, text) is
  'A decisão humana sobre a classe contábil (docs/05, "registro de override humano"). Append-only: '
...
--
-- O `docs/05` diz: *"o override vira sinal de calibração — onde humanos discordam
-- sistematicamente da máquina, ajusta-se regra/threshold (ou não se sobe o dial
...
comment on function fn_classe_contabil_concordancia(uuid) is
  'A concordância humano-máquina na classe contábil — o "sinal de calibração" que o docs/05 pede, em '
