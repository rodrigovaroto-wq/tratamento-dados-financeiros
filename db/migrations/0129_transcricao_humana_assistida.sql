...
--
-- O FECHAMENTO Nº 2 DO `docs/01`, QUE ERA O ÚNICO DOS OITO SEM CÓDIGO.
--
...
comment on column campo_extraido.origem_valor is
  ...
  'porque o arquivo não se lê — fechamento #2 do docs/01). Existe porque sem ela a primeira '
...
comment on function fn_registrar_transcricao_humana(uuid, jsonb, text, text) is
  'A SAÍDA do gate de captura (fechamento #2 do docs/01), que era o único dos oito fechamentos sem '
