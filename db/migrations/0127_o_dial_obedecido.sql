...
-- `docs/01` ser executada: subir dial de estágio interpretativo para auto-clear
...
-- permissão — é o default seguro que o fechamento #1 do `docs/01` exige
...
comment on function fn_dial_permite_auto(text, numeric) is
  ...
  'docs/01, para ela não existir copiada em quatro funções. Sem linha no dial devolve false: '
...
--
-- N0 no `docs/01` é "o estágio roda, registra a saída, mas NÃO influencia
...
comment on function fn_dial_influencia(p_estagio text) is
  'O resultado deste estágio pode chegar à fila de alguém? False só em N0, que o docs/01 define como '
...
begin
  ...
  --
  -- N1 é o TETO dela, e o docs/01 marca esse teto como "nunca autônomo". Então
