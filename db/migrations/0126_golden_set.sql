...
--                  docs/01 passa a ser EXECUTADA
--
-- O DEFEITO, E ELE É O MESMO DE SEMPRE NESTA CASA: A GUARDA PROMETIDA NÃO EXISTE.
--
-- `docs/01` fecha com uma "regra de ouro", em negrito e sem ressalva:
--
...
--      produto do dono, NÃO autonomia medida. docs/01 exige concordância contra
...
--
-- E O PROTOCOLO ESTÁ PRONTO DESDE 14/07. O `f0/06` foi fechado como v1 pelo dono:
...
-- `f0/06` como função, a tabela de teto do `docs/01` deixando de ser prosa e
...
-- com LGPD, e o `f0/06` já diz por que não se monta em documentação — "é tarefa
...
-- ESTRATO e ORIGEM.
--
-- O estrato é a exigência de amostragem do `f0/06`: "incluir digital, PDF nativo,
...
comment on type golden_estrato is
  'Qualidade de captura do documento (f0/06, amostragem estratificada). A métrica agregada sobre '
...
comment on type golden_origem is
  'real = documento de cliente. sintetico = book gerado (test-data/). As métricas que governam o '
...
--
-- `f0/06`: "golden set congelado por rodada de calibração; ampliado, não editado
...
-- Ampliar é rodada NOVA — e é assim que o `f0/06` manda o conjunto crescer.
-- -----------------------------------------------------------------------------
...
comment on table golden_rodada is
  'Rodada de calibração do golden set (f0/06). Congelada = não aceita mais rótulo; ampliar é rodada '
...
--
-- `f0/06`: "dois rotuladores independentes nos casos ambíguos; medir concordância
...
comment on table golden_rotulo is
  'O ground truth por rotulador (f0/06, "o que é rotulado"). Campo null = "este rotulador não '
...
-- -----------------------------------------------------------------------------
-- golden_campo — os `campos_chave` do `f0/06`, com TOLERÂNCIA declarada.
--
...
comment on table golden_campo is
  'Os campos_chave do f0/06: que valor o humano leu no documento, para a linha que a extração '
...
comment on column golden_campo.classe_contabil_correta is
  ...
  'da tabela de métricas do f0/06 — e o estágio classificacao_contabil tem teto N1 em docs/01, '
...
--
-- LGPD: `golden_documento` aponta para documento real de cliente, e o `f0/06`
...
-- inserir, e o `f0/06` teria a mesma sorte da regra de ouro: doutrina correta,
...
-- de 0,85, o `CUSTO_POR_MB_USD`). O `f0/06` dimensiona o N — "~20–30 documentos
...
comment on table golden_criterio is
  'O que "concordância alta e estável" (f0/06) significa em número, por estágio. n_minimo vem do '
  'f0/06 (~20-30 por tipo core); concordancia_minima é DECISÃO e entra em 0.95 porque é o '
...
comment on column golden_criterio.metrica is
  'Qual das cinco métricas do f0/06 governa este estágio. É o mapa que fn_golden_suficiente segue, '
...
insert into golden_criterio (estagio, metrica, nota) values
  ('classificacao_doc_checklist', 'f1_classificacao',
   'F1 da classificação doc->tipo (f0/06, linha 1 da tabela de métricas).'),
  ('extracao_identificadores', 'acuracia_identificadores',
   'Acurácia de tipo/período/entidade (f0/06, linha 2). A entidade casa por fn_mesma_entidade: '
   'duas grafias da mesma companhia não são erro.'),
  ('extracao_linhas_financeiras', 'acerto_campos',
   'Fração das linhas conferíveis cujo valor cai dentro da tolerância do rótulo (f0/06, linha 3). '
  ...
  ('reconciliacao_classe_a', 'nao_falso_positivo_classe_a',
   'f0/06 linha 5 pede a TAXA DE FALSO-POSITIVO; aqui ela entra invertida (1 - taxa) para que '
...
-- -----------------------------------------------------------------------------
-- A TABELA DE TETO DO docs/01 DEIXA DE SER PROSA.
--
-- `docs/01` tem uma tabela chamada "Regra de teto por natureza do estágio
...
comment on column estagio_autonomia.natureza is
  'docs/01, "regra de teto por natureza do estágio": deterministico = aritmética/integridade, cuja '
...
-- =============================================================================
-- AS CINCO MÉTRICAS DO f0/06
-- =============================================================================

-- -----------------------------------------------------------------------------
-- fn_golden_consenso — a base de todas as outras, e a regra que ela encarna é a
-- frase do `f0/06`: "se humanos discordam, a máquina não tem como acertar".
--
...
comment on function fn_golden_consenso(uuid) is
  ...
  'volta null com consenso=false, e as métricas o EXCLUEM: f0/06, "se humanos discordam, a máquina '
...
-- -----------------------------------------------------------------------------
-- fn_golden_inter_avaliador — a concordância entre HUMANOS (f0/06, protocolo de
-- rotulagem, item 1).
--
...
comment on function fn_golden_inter_avaliador(uuid, golden_origem) is
  'Concordância entre rotuladores, por campo (f0/06). Conta só documento com 2+ rotuladores — com um '
...
-- (f0/06, linha 1). Por TIPO, que é como o protocolo pede e como a decisão de
...
comment on function fn_golden_classificacao(uuid, golden_origem) is
  'Precisão/recall/F1 da classificação doc->tipo, POR TIPO (f0/06). O universo de tipos é a união da '
...
-- -----------------------------------------------------------------------------
-- fn_golden_identificadores — acurácia de tipo / período / entidade (f0/06,
...
comment on function fn_golden_identificadores(uuid, golden_origem) is
  'Acurácia de tipo/período/entidade separadamente (f0/06). Separados porque as causas de erro são '
...
-- -----------------------------------------------------------------------------
-- fn_golden_campos — erro de extração de campos financeiros (f0/06, linha 3) E a
...
comment on function fn_golden_campos(uuid, golden_origem) is
  'Erro de extração de campo financeiro (f0/06) com AUSENTE separado de ERRADO — perda silenciosa é '
...
-- -----------------------------------------------------------------------------
-- fn_golden_classe_a — taxa de falso-positivo da reconciliação Classe A (f0/06,
...
comment on function fn_golden_classe_a(uuid) is
  'Taxa de falso-positivo da reconciliação Classe A (f0/06, linha 5). Única das cinco métricas que '
...
-- -----------------------------------------------------------------------------
-- fn_golden_cobertura — o dimensionamento do `f0/06`, medido.
--
-- OS "TIPOS CORE" NÃO ENTRAM COMO LISTA. O `f0/06` define core como "os 8 do Kit
...
--
-- E `granularidade` responde de graça a nuance que o `f0/06` levanta: os tipos de
...
comment on function fn_golden_cobertura(uuid, golden_origem) is
  'Quantos documentos rotulados a rodada tem por tipo CORE, contra o alvo do f0/06 (~20-30). Core sai '
  ...
  'demorar a juntar 20 é esperado, e o f0/06 diz isso.';
...
-- `f0/06` deixa de ser um losango de fluxograma.
--
-- DUAS REGRAS QUE NÃO SÃO ÓBVIAS E SÃO AS QUE IMPORTAM:
--
-- 1. O TIPO MAIS FRACO GOVERNA. O dial é por ESTÁGIO (0001/0002) e o `f0/06`
...
comment on function fn_golden_suficiente(text, uuid) is
  'A pergunta do laço de calibração do f0/06 ("concordância alta e estável?"), respondida em número. '
  'O TIPO MAIS FRACO governa: o dial é por estágio e o f0/06 raciocina por tipo, e autonomia por '
...
-- -----------------------------------------------------------------------------
-- fn_mudar_dial — a regra de ouro do `docs/01` passa a ser executada aqui.
--
