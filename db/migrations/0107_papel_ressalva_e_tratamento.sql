-- =============================================================================
-- Migration 0107 — A máquina de estado da pendência fica INTEIRA (f0/04)
--
-- `f0/04` define seis estados desde a F0, e até aqui o código escrevia três:
--
--     aberta            ✅ o motor abre
...
-- ---------------------------------------------------------------------------
-- `f0/04`: "Exige papel SÊNIOR + motivo obrigatório + data de expiração." O
...
-- Rejeitar uma NÃO-SOBREPUJÁVEL — a lista fechada de f0/04, a alavanca mais forte
...
comment on table usuario_papel is
  'Papel por e-mail (f0/04: ressalva exige sênior). Quem não está aqui é `analista` — o default é '
...
-- -----------------------------------------------------------------------------
-- fn_ressalvar_pendencia — o caminho que `f0/04` especificou na F0 e que nunca
-- existiu em código.
-- -----------------------------------------------------------------------------
...
comment on function fn_ressalvar_pendencia(uuid, text, text, timestamptz) is
  'Aceita a pendência COM RESSALVA (f0/04): exige papel sênior, motivo, data de expiração futura, '
...
--
-- NÃO LIBERA O PORTÃO, e isso é o ponto: `f0/04` conta 'aberta',
...
comment on function fn_tratar_pendencia(uuid, text, text, text) is
  'Move a pendência entre os estados de TRATAMENTO de f0/04 (aberta / em correção interna / '
