...
-- é detalhe de arrumação: o portão do `db/test/run.sh` REPROVA prefixo
...
-- do `db/README.md` passa a mentir em silêncio. O conteúdo é o do PR #135.
--
...
begin
  ...
  -- ele fazer) e depois reaplicasse a lista de comandos do `db/README.md` —
...
-- `db/varredura_linha_exigida.sql`. Ela é idempotente (recomputar é o que o
-- pipeline já faz a cada extração), aceita rodar num caso só, e o
-- `db/README.md` a lista como passo OPCIONAL, depois da migration. Sem ela,
