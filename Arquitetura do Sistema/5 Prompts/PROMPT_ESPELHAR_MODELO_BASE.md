...
réplica fiel do **Modelo Base** — a planilha de referência do dono, em `Arquitetura do Sistema/6 Referência/modelo-base.xlsx`.
...
Medições feitas em 2026-08-05 com `Arquitetura do Sistema/6 Referência/mapear-xlsx.py` (§6), contra
`Arquitetura do Sistema/6 Referência/modelo-base.xlsx` e contra um export completo real do caso de teste v35.
...
**`Arquitetura do Sistema/6 Referência/mapear-xlsx.py`** — lê qualquer `.xlsx` como ZIP de XML.

```bash
# resumo de todas as abas: fórmulas, validações, cond., merges, congelamento
python3 "Arquitetura do Sistema/6 Referência/mapear-xlsx.py" Arquitetura do Sistema/6 Referência/modelo-base.xlsx
...
```bash
TEST_DB=tdf_v35 PGDATABASE=postgres Supabase/test/run.sh          # migrations + suíte
...
Produza `Arquitetura do Sistema/6 Referência/MAPA_MODELO_BASE.md`, versionado. Para **cada uma das 14 abas**:
...
Produza `Arquitetura do Sistema/6 Referência/CONFORMIDADE.md`: uma tabela por aba, e dentro dela uma linha por
...
3. **Contadores nunca caem.** Estado atual: `N8N/test` **176** · `verificar-export.mts` **435** ·
   `Supabase/test/run.sh` **49 migrations / 324 asserts** · `test/e2e` **27**. Se um número cair, você
   apagou cobertura.
...
5. **Migrations**: faixa `0100+` é a do colaborador; a última é a `0104`. Confira `Supabase/migrations/`
   antes de criar, registre no `Supabase/README.md` **na tabela E na lista de comandos** (o `run.sh`
...
9. **Custo**: `Dados de Teste/book-vertentes` e a fixture do v35 exercitam tudo **sem IA**. Teste ao
...
- **Não remova as 15 abas nossas** (`Resumo`, `Dados (linha a linha)`, `Balanço`, `DRE`,
  ...
  `Arquitetura do Sistema/2 Especificação/f0/07` é que o dado curado e rastreável não some da entrega. O Modelo Base não as tem porque
...
1. **`Arquitetura do Sistema/6 Referência/MAPA_MODELO_BASE.md`** — o mapa completo das 14 abas (fase 1).
2. **`Arquitetura do Sistema/6 Referência/CONFORMIDADE.md`** — a tabela de vereditos, atualizada a cada PR (fase 2).
...
2. o resultado de `python3 "Arquitetura do Sistema/6 Referência/mapear-xlsx.py" Arquitetura do Sistema/6 Referência/modelo-base.xlsx` —
