...
| **Estado** | 🔴 **6 de 36 com exigência VIVA** (lida por alguma checagem, `origem='codigo'`) — esse número **não mudou** com a F2.1. O que mudou: as `proposta` passaram de 3 para **12** (as 3 da `0113` + as 9 da `0185`, NÃO APLICADA). Exigência `proposta` nomeia a ausência e não é lida por checagem nenhuma — chamá-la de "viva" inflaria o progresso, que é a regra 1 aplicada ao próprio roadmap. **Medição (22/09/2026): os consumidores ATUAIS concluem entre 8,3% e 67,8%** por caso × entidade (`caixa_bp_vs_fluxo` fora da conta: é nome morto, renomeado em 27/07) — `mutuos` 8,3%, `caixa_bp_fluxo` 8,6%, `despfin_dre_vs_divida` 13,8%, `receita` 21,1%, `intragrupo` 33,3%, `ativo_passivo_pl` 57,1%, `secao_fecha` 67,8%. Acrescentar exigência NOVA antes de saber por que os atuais não concluem é construir em andar não verificado |
...
| **F2.1 — MEDIDA CONTRA PRODUÇÃO EM 21/09/2026, E REPROVADA** | **NÃO APLICAR a `0185` como está.** Simulação somente-leitura do predicado de `fn_exigencias_do_caso` com os localizadores da `0185` sobre o banco real (que está na `0181`): 190 documentos dos nove tipos em 14 casos, 64 pares caso×tipo com conteúdo, **17 abririam pendência — e os 17, olhados um a um, têm o dado**. Causa raiz, que não é escolha de termo: **em relatório ITEMIZADO o conceito não está no rótulo — o rótulo é o item, e o conceito é o tipo do documento.** No mandato real, o AGING_AP tem chaves `41518 - WELLA BRASIL LTDA.` e o ESTOQUE tem 484 linhas tipo `2500 - ASSALA PRIME`; em CONTINGENCIAS as chaves são descrições de processo e as seções são `Trabalhista`/`Cível`/`Tributário - DIFAL`. E o outro lado erra junto: 22 dos 47 "passam" por uma única linha residual (`Demais fornecedores (184 credores)`), que é o agregado que o aging justamente não abre. **Redesenho necessário**: para tipo itemizado a pergunta é estrutural (quantas linhas com valor, o eixo esperado do relatório), não lexical — ou nenhuma, assumindo que `item_sem_conteudo` (0036) já cobre "chegou vazio" |
| **Achado de método (vale para QUALQUER portão futuro)** | A `secao` **não é estável entre versões da extração**: o mesmo book canastra tem `secao` NULA nas ingestões antigas e preenchida nas novas. A `fixture_book_canastra.sql` tem `secao`, produção às vezes não — então o teste que a revisão exigiu (medir contra a fixture) passava nos seis tipos e ainda assim a fatia era falsa em produção. Terceira ocorrência da lição de `.claude/memory/portao-mede-a-entrada-de-producao.md`. **Localizador que depende de `secao` se apoia na versão do extrator, não no documento** |
| **DECISÃO DO DONO (21/09/2026)** | Os nove tipos ficam **COMPLEMENTARES**. Perguntado se aging/extrato deveriam subir para o Kit Básico, o dono respondeu "não, manter como complementar" — fecha a pergunta que `2 Especificação/f0/03_taxonomia_reestruturacao.md` deixava para a v2 |
