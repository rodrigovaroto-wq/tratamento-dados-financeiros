...
**Insumo:** `Dados de Teste/book-canastra` (38 PDFs sintéticos), conferido contra o
`pdf/GABARITO.json` regerado nesta sessão.

> **Como esta análise foi feita.** Não por leitura de tela: por consulta ao banco de
> produção (`Supabase/diagnostico_rodada.sql`, `Supabase/pendencias_do_mandato.sql` e derivadas) e
...
`Supabase/diagnostico_rodada.sql` diz que este projeto já pagou caro.
