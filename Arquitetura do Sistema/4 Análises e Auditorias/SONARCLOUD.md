...
tem autoridade para opinar sobre o resto do SQL — e é por isso que `Supabase/` sai do
escopo em vez de ganhar 1.726 exceções uma a uma.

Os **9 BLOCKER** da análise seguem a mesma linha: são `DELETE`/`UPDATE` sem
`WHERE` no *teardown* de `Supabase/test/`, que roda contra banco local recriado do zero
a cada execução do `run.sh`. É o comportamento pretendido.

E há a parte que nenhum ajuste de regra resolve: `Supabase/migrations` é **imutável**
(migration aplicada não se reescreve) e `Supabase/schema.sql` é **gerado** pelo
...
```properties
sonar.exclusions=db,Dados de Teste/capturas
```
...
| `Verificação/run.mts` | `versaoNova`, `Map` preenchido e nunca lido |
