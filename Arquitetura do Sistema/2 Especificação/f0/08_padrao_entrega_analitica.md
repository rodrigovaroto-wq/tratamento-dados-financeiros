...
Fundamentado no material `Arquitetura do Sistema/6 Referência/Embasamento sobre Contabilidade` (bibliografia curada pelo dono) e
...
`Arquitetura do Sistema/2 Especificação/f0/07` mantém FORA do escopo) o que importa é a camada **(c) Análise de demonstrações**:
...
- **Schilit, "Financial Shenanigans"** — *red flags* de qualidade: CFO divergindo do Lucro
  ...
  parcialmente cobertos pelo motor de reconciliação (Classe A/B, `Arquitetura do Sistema/2 Especificação/04`).
...
## Reconciliação com a doutrina (por que isto NÃO viola `Arquitetura do Sistema/2 Especificação/f0/07`/`Arquitetura do Sistema/1 Visão e Doutrina/01`)
...
**margem** já entregues (`Arquitetura do Sistema/2 Especificação/f0/07`, emenda 2026-07-22) e da lista de "próximos passos" do handoff
...
4. **Margens** (Bruta/Operacional/Líquida) na DRE — já entregues (emenda de `Arquitetura do Sistema/2 Especificação/f0/07`), mantidas.
...
- **Dívida Líquida, Dívida Líquida/EBITDA, Dívida Líquida/PL** — exigem dívida bruta e caixa
  isolados (e EBITDA, que a DRE hoje não traz — D&A não vem como linha isolada, `Arquitetura do Sistema/2 Especificação/f0/07` cont.⁴).
