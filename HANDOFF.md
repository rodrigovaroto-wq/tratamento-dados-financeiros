...
- **Revisão no mesmo dia** (feedback do dono: a primeira versão saiu como lista achatada —
  "quero formatado igual um balanço ou DRE... use o padrão do mercado"). `src/lib/
  statement-templates.ts` define o **layout padrão de mercado** (Ativo/Passivo/PL hierárquico
  no Balanço — CPC/prática brasileira; cascata Receita→Lucro Líquido na DRE; Atividades
  Operacionais/Investimento/Financiamento no Fluxo de Caixa — método indireto, CPC 03) para
  `Balanço`/`DRE`/`Fluxo de Caixa`/`Combinado` (esta reaproveita o template do Balanço).
  Colunas = entidade × período (uma demonstração real, lado a lado); linhas = contas do
  template. Casamento chave extraída→conta do template usa a MESMA técnica determinística de
  `fn_valor_conceito` (`db/migrations/0009`) portada para TypeScript — nunca soma/calcula um
  subtotal novo, só recoloca o que a IA já extraiu no lugar certo (anti-ancoragem: não
  inventamos números). Contas extraídas que não batem com nenhuma linha do template vão para
  um apêndice "Outras contas identificadas" no fim da aba — nada desaparece silenciosamente.
  Proveniência (arquivo/página/confiança/status/versão da taxonomia) virou **comentário da
  célula** em vez de colunas auxiliares (as colunas agora são entidade×período). Faturamento/
  Dívida/Fluxo Projetado continuam em listagem simples (já são, por natureza, série/tabela).
- A lógica de montagem do workbook é uma **função pura** (`buildExportWorkbook` +
  `casarPadrao`, sem Supabase/Next.js) — testada isoladamente nesta sessão com dados
  sintéticos via `tsx`: reabre o `.xlsx` gerado com `exceljs` e confere célula a célula —
  inclusive um cenário com a MESMA conta em dois períodos diferentes da mesma entidade
  (funcionou), e a disambiguação "Total do Ativo" vs. "Total do Ativo Circulante/Não
  Circulante". **Achou e corrigiu um bug real de sobreposição de padrão** durante o teste: o
  padrão de "Total do Patrimônio Líquido" também casava com a linha combinada "Total do
  Passivo e do Patrimônio Líquido" (ambas contêm "total"+"patrimonio liquido" como substring)
  — corrigido com um `exclude: ['passivo']` no padrão da linha isolada. A rota em si (busca
  via Supabase) **não foi exercitada contra um projeto real** — só a montagem do Excel.
...
| Export Excel — Balanço/DRE/Fluxo de Caixa/Combinado: layout PADRÃO DE MERCADO (linhas = contas do template contábil, colunas = entidade×período), casamento por termos determinístico (nunca soma/calcula subtotal novo); Faturamento/Dívida/Fluxo Projetado continuam em listagem simples (já são série/tabela por natureza) | `portal/src/lib/statement-templates.ts` |
...
1. **Testar o export com um caso real** do dono (aplicar `0011`, subir documentos reais,
   ...
   real ainda, só a montagem do workbook em si). **Validar com o time de análise** se a lista
   de contas do template (`statement-templates.ts`) bate com o que eles esperam ver — foi
   montada por bom senso contábil (CPC/prática de mercado), não checada com quem vai usar.
...
> (`fn_aceitar_extracao`, `0011`), o export (`portal/src/lib/export.ts`) e o layout padrão de
> mercado do Balanço/DRE/Fluxo de Caixa (`portal/src/lib/statement-templates.ts`) já existem
> nessa primeira fatia — ler os três antes de refinar a granularidade do aceite, ajustar contas
> do template (a lista foi feita por bom senso contábil, não validada com o time de análise
> real ainda), ou adicionar um template padronizado para Faturamento/Dívida.
