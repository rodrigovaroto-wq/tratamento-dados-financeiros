...
O `book-vertentes` continua valendo — ele é o insumo do `verificar-export`, do `Supabase/test` e do e2e, e
...
| Nomes de arquivo | todos na notação de Arquitetura do Sistema/2 Especificação/f0/03 | **metade como o cliente manda** (ano solto, sem período, `Doc1.pdf`) |
...
isto é, o equivalente do `Supabase/test/gerar_fixture.py`, que converte o book em linhas de
`campo_extraido` para o `Supabase/test`, o `verificar-export` e o e2e rodarem contra ele. Essa é a fatia
