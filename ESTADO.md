...
| | |
|---|---|
| **Última migration** | `Supabase/migrations/0180_o_perimetro_que_o_combinado_nao_tinha.sql` — fatia 1.4 do plano F1 (seção 12.2 do roadmap). Tabela nova `perimetro(caso, entidade, escopo, desde, ate)`: quem entra no COMBINADO de um caso — antes desta migration não havia registro nenhum disso, só o que um documento COMBINADO diz sobre si mesmo (`autoridade_combinado`, 0155). Caminho de escrita explícito `fn_perimetro_definir_escopo` (humano, nunca inferência automática — a troca de escopo no meio do mandato FECHA o intervalo anterior com `ate = nova_desde - 1`, nunca sobrescreve, porque um perímetro sem data mente sobre o exercício anterior) e consumidor mínimo de leitura `fn_perimetro_vigente` (quem está no escopo numa data). NÃO deriva escopo de `entidade.papel_no_grupo` (0179) — ligar as duas fatias é decisão de F4 (consolidação), documentado no cabeçalho da migration para a sessão seguinte não reabrir sem contexto. 17 asserts novos em `Supabase/test/perimetro.test.sql`, 4 dos 17 MEDIDOS reprovando (não estimados) contra o estado sem o fechamento do intervalo anterior em `fn_perimetro_definir_escopo` — os outros 13 passam com ou sem a correção (controles genuínos ou blocos que não discriminam esta regressão específica; ver o cabeçalho do arquivo de teste). **Ainda não aplicada em produção** — só a sonda responde: `select chave, migration, tipo, objeto, presente, detalhe, porque from fn_instalacao_conferir() where not presente order by 1;`. A `0178` e a `0179` seguem como estavam: escritas, não aplicadas. |
| **Aplicadas no Supabase** | **até a `0177`** — aplicadas em 16/09/2026, pela API de gerenciamento do Supabase (a sessão não alcança a porta do Postgres direto; ver `.claude/memory/` sobre o ambiente); MEDIDO, não suposto, ver o histórico abaixo. **A `0178` (fatia 1.2), a `0179` (fatia 1.3) e a `0180` (fatia 1.4) ainda NÃO foram aplicadas** — escrita ≠ aplicada é doutrina deste projeto, e quem aplica é uma sessão seguinte, contra a sonda. |
