...
| | |
|---|---|
| **Última migration** | `Supabase/migrations/0178_o_titulo_da_planilha_nao_e_pessoa_juridica.sql` — fatia 1.2 do plano F1 (seção 12.2 do roadmap). `fn_upsert_entidade` (0030) não tinha guarda entre "nome próprio de empresa" e "título de coluna/aba/arquivo": MEDIDO em produção (mandato "AMO teste 00", 18/09/2026), 4 das 13 entidades do caso NÃO são entidades — `Empresas`, `Vencidos`, `Status Extratos`, `Controle Extratos Ofx` —, todas sem CNPJ e com 1 documento, contra as 8 reais, todas com CNPJ. A guarda nova (`fn_entidade_nome_parece_titulo_ou_arquivo`, léxico de referência = `Supabase/test/perimetro-inventario.mjs`/`PADRAO_TITULO_OU_ARQUIVO`, fatia 1.1) exige a CONJUNÇÃO sem-CNPJ + léxico + primeiro documento — nunca "sem CNPJ" sozinho, para não reabrir o defeito que a 0175-0177 corrigiram no balcão ambíguo (entidade real sem CNPJ tratada como descartável). Decisão do dono: as 4 entidades já existentes NÃO são deletadas (perde proveniência) nem fundidas automaticamente (fundir errado é pior) — são MARCADAS via `fn_pendencia_entidade_nome_suspeito` (pendência `entidade_incorreta`, motivo `entidade_nome_suspeito:<id>`, idempotente) para revisão humana; a migration inclui o backfill para quem já existir no banco em que ela rodar. 12 asserts novos em `Supabase/test/entidade_titulo_suspeito.test.sql`, com 5 deles medidos reprovando contra o estado da 0177 sem a guarda antes de escrever a correção. **Ainda não aplicada em produção** — só a sonda, contra o banco em que se está conectado, responde isso: `select chave, migration, tipo, objeto, presente, detalhe, porque from fn_instalacao_conferir() where not presente order by 1;`. A 0177 (guarda do balcão bidirecional) segue como estava: aplicada e conferida — ver histórico abaixo. |
| **Aplicadas no Supabase** | **até a `0177`** — aplicadas em 16/09/2026, pela API de gerenciamento do Supabase (a sessão não alcança a porta do Postgres direto; ver `.claude/memory/` sobre o ambiente); MEDIDO, não suposto, ver o histórico abaixo. **A `0178` (fatia 1.2 do F1) ainda NÃO foi aplicada** — escrita ≠ aplicada é doutrina deste projeto, e quem aplica é uma sessão seguinte, contra a sonda. |
