...
| | |
|---|---|
| **Última migration** | `Supabase/migrations/0176_o_balcao_nao_absorve_quem_e_confirmado.sql` — **a 0175 (já commitada, `f549842`) fez o balcão ambíguo tentar o CNPJ, e uma revisão independente (`revisor-defeito-silencioso`, dois bancos Postgres, um com a 0175 e um sem) mediu CINCO defeitos que sobrevivem à leitura.** **CRÍTICO:** desde a 0175 o balcão (identidade EXPLICITAMENTE desconhecida) pode ter CNPJ, e o resto do sistema desde a 0169 trata "mesmo CNPJ" como identidade CONFIRMADA sem olhar nome — dois buracos de absorção silenciosa. (A) `fn_upsert_entidade` ramo (0): um documento NOVO com o CNPJ de um balcão já existente era registrado DENTRO dele sem checar `fn_entidade_e_balcao_ambiguo` — MEDIDO: "OUTRA METALURGICA NOVA SA" absorvida, o caso ficando com 3 entidades em vez de 4. (B) `fn_entidade_aprender_cnpj` (chamada pelo ramo do balcão desde a 0175): uma entidade CONFIRMADA (documento e nome próprios) que trouxesse o mesmo CNPJ era FUNDIDA e DELETADA dentro do balcão — MEDIDO: uma "VERTENTES METALURGICA LTDA" desaparecia, o caso caindo de 4 para 3 entidades, SEM pendência nenhuma acusando (rastro só em `evento_auditoria`). **A CORREÇÃO:** um balcão só pode ser o lado que recebe/sobrevive via CNPJ quando quem está do outro lado TAMBÉM é balcão (a convergência balcão↔balcão que é o ponto da 0175, preservada); do contrário, a colisão vira pendência nova (`fn_pendencia_cnpj_colide_balcao`, função nova, motivo `entidade_cnpj_colide_balcao:<balcão>`) e nada funde nem atribui — o caminho normal decide como decidiria se o balcão nunca tivesse este CNPJ. **ALTO:** `v_entidade_atual_nome`, lido ANTES do bloco do balcão e nunca reconferido, alimentava a pendência `diagnostico:entidade_ambigua_respondida` depois que o bloco podia ter FUNDIDO (linha deletada) ou RENOMEADO a entidade — MEDIDO: a pendência citava "OMNIBEAUTY DESENVOLVIMENTO E GESTAO" (nome do balcão fundido, já apagado) atribuída ao balcão sobrevivente, que se chama "...GESTAO DE". Corrigido relendo `razao_social` depois do bloco. **MÉDIO 1:** o renomeio dentro do balcão passava o `p_cnpj` BRUTO para `fn_entidade_talvez_renomear`, mesmo quando `fn_entidade_aprender_cnpj` voltou SEM gravar nada (outro CNPJ já gravado, ou a recusa do CRÍTICO) — MEDIDO: entidade com `cnpj = 11222333000181` "renomeada por CNPJ" com o evento citando `{"cnpj": "36193378000104", ...}`, estranho à entidade. Corrigido: só renomeia com o CNPJ que o `aprender` de fato confirmou NESTA entidade. O MESMO padrão existe no ramo `else` (pré-existente desde a 0173) — FORA do escopo desta fatia, registrado para o `MAPA_DE_EXECUCAO.md`. **MÉDIO 2:** "todo balcão ambíguo nasce sem CNPJ" (cabeçalho/`porque` da 0175) é FALSO desde a 0170 — `fn_upsert_entidade` grava CNPJ no balcão nascido por ambiguidade quando a CLASSIFICAÇÃO chega com `p_cnpj` — MEDIDO. Não quebra a convergência (a guarda do CRÍTICO em `fn_upsert_entidade` impede uma segunda entidade nascer com um CNPJ que um balcão já detém); só o TEXTO foi corrigido (`porque` do requisito `balcao_ambiguo_aprende_cnpj`). **MÉDIO 3:** dois marcadores de sonda colidiam (`position()` só vê presença, não conta ocorrências) — `balcao_ambiguo_aprende_cnpj` tinha marcador frágil (`'if p_cnpj is not null then'`, casaria com qualquer bloco futuro com essa condição) e `renomeio_no_diagnostico` casava com DUAS chamadas idênticas (ramo `else` da 0173 e ramo do balcão da 0175) — MEDIDO por `grep -c`: `2` ocorrências antes, `1` depois. Os dois marcadores foram trocados por texto amarrado ao código do MÉDIO 1 (que torna as duas chamadas de `fn_entidade_talvez_renomear` textualmente diferentes). **Invariante novo** (`Supabase/test/balcao_nao_absorve_confirmada.test.sql`): cobre os cinco defeitos, com o cenário da "metalúrgica"/CNPJ de rodapé de contador sendo FIXTURE declarada (não há dado de produção desse par específico — regra 4 do CLAUDE.md), e reconfirma que `Supabase/test/balcao_ambiguo_e_cnpj.test.sql` (18 asserts da 0175) continua verde. Antes dela: `Supabase/migrations/0175_o_cnpj_tambem_resolve_o_balcao.sql` — **a LACUNA que a `0174` já declarava no fim do próprio arquivo de teste** (`cnpj_identidade.test.sql`, bloco 11: "o ramo balcão ambíguo nunca chama fn_entidade_aprender_cnpj"). **Medido em produção** (mesmo caso `bf0246bb`, teste 143): depois de republicar até a `0173`, `evento_auditoria` mostrou DOIS `entidade_ambigua` NOVOS em vez de uma fusão — as mesmas variantes de sempre, ainda separadas. **A CAUSA:** dentro de `fn_registrar_diagnostico`, o ramo `if fn_entidade_e_balcao_ambiguo(...)` (`0162`) só sabia perguntar ao NOME — nunca chamava `fn_entidade_aprender_cnpj`, mesmo com `p_cnpj` presente. Todo balcão nasce sem CNPJ; quando dois documentos da mesma empresa caem em DOIS balcões diferentes, nenhum converge, porque a única saída do ramo é pendência para humano. **Medido contra o estado da `0174`, sem a correção:** dois documentos com o mesmo CNPJ, um em cada balcão, terminam com 4 entidades e 2 pendências `entidade_ambigua` abertas — nenhuma exceção, ausência silenciosa (ao contrário do crash da `0174`). **A CORREÇÃO:** tenta o CNPJ ANTES do nome dentro do ramo do balcão — `fn_entidade_aprender_cnpj` (`0169`/`0174`) grava no próprio balcão (sem outra dona) ou FUNDE com quem já tinha o CNPJ (usando o RETORNO, mesmo cuidado da `0174`), com o renomeio pela MESMA `fn_entidade_talvez_renomear` da `0173`; só cai na lógica de nome (inalterada) se o balcão ainda for ambíguo depois disso. **Invariante** (`Supabase/test/balcao_ambiguo_e_cnpj.test.sql`), 18 asserts: dois balcões nascidos de variantes reais da OMNIBEAUTY (mais um corte de token, mesmo algoritmo) convergem em UM quando o mesmo CNPJ chega por cada um pelo diagnóstico — 4 entidades → 3, 2 pendências → 1 (a do sobrevivente, cujo NOME continua ambíguo por desenho), com rastro de `fn_fundir_entidade` e da guarda de homônima (`0171`) recusando o renomeio dos dois lados. **Medido com a correção desligada** (revertendo a função para o corpo da `0174`): 10 asserts passam e o 11º reprova com `FALHOU: as QUATRO entidades caem para TRÊS ... — 4 entidade(s) no caso (esperado 3)`. Antes dela: `Supabase/migrations/0174_aprender_cnpj_que_ja_tinha_dona_funde.sql` — **medido em PRODUÇÃO, não em fixture:** o dono rodou o lote real da OMNIBEAUTY (caso `bf0246bb`, teste 143) depois de republicar o workflow com as migrations até a 0173, e `evento_auditoria` mostrou DOIS `entidade_ambigua` novos em vez de uma fusão — as mesmas quatro variantes de sempre, ainda separadas. Reproduzido aqui com os MESMOS nomes reais e o MESMO CNPJ, na ordem em que o pipeline de fato executa: a chamada que crasha é `fn_entidade_aprender_cnpj`, com `ERROR: duplicate key value violates unique constraint "entidade_caso_cnpj_unico"`. **A CAUSA:** o CNPJ chega pelo DIAGNÓSTICO (0172/0173) só DEPOIS que o documento já foi registrado e a entidade já resolvida por NOME, sem CNPJ nenhum disponível ainda. Quando duas variantes da MESMA empresa chegam assim, cada uma vira uma entidade PRÓPRIA sem CNPJ; o primeiro documento confirmado aprende o CNPJ sem problema, o SEGUNDO (de uma entidade DIFERENTE) tenta gravar o MESMO CNPJ e o índice único (0169) rejeita — e `fn_entidade_aprender_cnpj` só sabia UPDATE ou nada, nunca fundir. Como o diagnóstico roda para TODO documento, isto não era caso de borda: era o destino de qualquer par de documentos da mesma empresa cujos nomes não casassem ANTES de o CNPJ aparecer — o cenário que motivou a 0171 inteira, pela porta errada. **A CORREÇÃO:** `fn_entidade_aprender_cnpj` muda de `void` para `uuid` (DROP + CREATE — `create or replace` não permite mudar tipo de retorno) e, quando o CNPJ já pertence a OUTRA entidade do caso, FUNDE (`fn_fundir_entidade`) em vez de gravar, depois chama `fn_entidade_talvez_renomear` para a sobrevivente adotar o nome mais completo — reaproveitando as MESMAS três guardas da 0171/0173, sem duplicar lógica. **Os DOIS chamadores** (`fn_upsert_entidade` ramo (1) e `fn_registrar_diagnostico`) tinham de trocar `perform` pelo RETORNO: sem isso a variável local ficaria com o id de uma linha DELETADA — em `fn_upsert_entidade` isso quebraria a FK de `documento.entidade_id` no INSERT seguinte (um crash NOVO, pior que o consertado). **Medido (dois cenários reais, os dois passam):** nomes sem raiz comum (MARCAS LTDA × SURUBIJU) fundem e a recusa de renomeio fica registrada com rastro; nomes truncados (o vazio "…DE" × "…DE MARCAS LTDA") fundem E a sobrevivente adota o nome completo, não importa qual das duas já tinha o CNPJ. Antes dela: `Supabase/migrations/0172_o_cnpj_pelo_caminho_que_roda_sempre.sql``0172_o_cnpj_pelo_caminho_que_roda_sempre.sql` — **a revisão do diff achou que o CNPJ só chegava na METADE do lote.** A fatia anterior fez a IA ler o CNPJ na chamada de CLASSIFICAÇÃO — e essa chamada **só roda no ramo de fallback**, quando o nome do arquivo não resolve tipo+período com confiança ≥ 0,70. **MEDIDO no book que o repositório versiona** (`medir-custo-book.mjs`): **19 de 38 documentos** do `book-canastra` passam por ela; os outros 19 chegariam com `cnpj` NULO **e sem sintoma nenhum** — a identidade continuaria sendo o nome, na metade do lote, com todo o código de identidade instalado e metade dele nunca executado (regra 7). **A CORREÇÃO:** o caminho que roda SEMPRE é o do DIAGNÓSTICO — a chamada de extração é a única leitura de conteúdo garantida para todo documento, é ela que devolve `diagnostico.entidade`, e é por ela que os nomes contaminados NASCEM. `fn_registrar_diagnostico` (11 → 12 args, com `drop function` da de 11 pela mesma armadilha de overload da 0118/0170) recebe `p_cnpj` e o repassa. **Os DOIS caminhos ficam**, e é de propósito: a classificação chega ANTES (evita a entidade nascer com o nome errado), o diagnóstico chega depois e cobre os 100%; a regra de aprendizado da 0169 (nunca sobrescreve CNPJ já gravado) faz a ordem não importar. **No n8n:** `extractionSchema()` e o `SYSTEM_PROMPT` ganham `cnpj` (com o aviso do rodapé do contador), o parse repassa, e o nó `Registrar Diagnostico` passa `p_cnpj` por NOME — o `$12` já é dos FATOS, e reordenar os binds quebraria `fn_registrar_fatos` em silêncio. **Portão novo** (`workflow-sim.test.mjs`): afirma o COMPORTAMENTO — algum nó que grava CNPJ tem de estar FORA do ramo condicional — e reprova, medido, quando o fio do diagnóstico é desligado. Antes dela: `Supabase/migrations/0171_o_cnpj_tambem_renomeia.sql` — **decisão do dono**, tomada em resposta direta: quando o CNPJ funde duas variantes de nome, ela deveria também adotar o nome MAIS COMPLETO entre os que já apareceram — antes ela só fundia e mantinha o nome de quem chegou primeiro, que pode ser o truncado ou o contaminado pelo endereço. **TRÊS SINAIS em ordem de força**, cada um só decide quando o de cima empatou: (1) **cara de endereço colado** (vírgula + número no fim — o padrão exato de "…SURUBIJU, 1930") **nunca vence, mesmo sendo mais longo** — é o sinal que existe porque comprimento cru sozinho erraria: "SURUBIJU, 1930" (55 chars) é mais longo que "MARCAS LTDA" (52) e é o nome ERRADO; (2) **sufixo societário** (LTDA, S.A., …, o MESMO padrão que `fn_entidade_canonica` já usa) vence quem não tem; (3) **comprimento cru** só desempata por último, entre dois nomes que empataram nos dois sinais acima — é o critério que a 0168 já usa sozinho para a fusão SEM CNPJ, e continua certo ali porque não há CNPJ provando identidade. **ESCOPO: só o ramo (0)** — CNPJ igual. Os ramos de casamento aproximado (0153/0168) continuam sem renomear ao decidir: ali o CNPJ não provou nada. **Invariante** (`Supabase/test/cnpj_renomeia.test.sql`), 9 asserts em 6 blocos, com os quatro nomes reais e o CNPJ real: (1)/(2) o nome contaminado não vence em **nenhuma** das duas ordens de chegada — o bloco 2 é o que reprova com a correção desligada, porque sem ela o primeiro a chegar sempre fica; (3) sem sufixo dos dois lados, quem não tem cara de endereço vence mesmo mais curto; (4) sufixo vence o truncado; (5) o rastro (`entidade_renomeada_por_cnpj`) só nasce quando o nome REALMENTE muda, e um terceiro documento pior não desfaz o renomeio; (6) sem CNPJ, a 0168 está intacta (regressão). **MUDANÇA NÃO MEDIDA CONTRA LOTE REAL** — limite desta sessão, sem credencial de OpenAI nem acesso ao n8n; medida contra os nomes reais já fornecidos, determinística, sem IA. Antes dela: `Supabase/migrations/0170_o_cnpj_atravessa_a_porta_de_entrada.sql` — **a 0169 nasceu DORMENTE, e esta é o fio.** `fn_registrar_documento` — a porta por onde todo documento entra — chamava `fn_upsert_entidade` com DOIS argumentos, e o terceiro ficava no default nulo: as três regras de identidade da 0169 nunca disparariam em produção, e **o único sintoma seria não haver sintoma** (regra 7 — estágio ligado e estágio que não roda têm a mesma aparência). A migration reemite `fn_registrar_documento` INTEIRA, verbatim do `schema.sql` vigente, com DUAS mudanças e nenhuma outra: `p_cnpj text default null` no fim da assinatura, e a chamada passando `p_cnpj`. **16 → 17 argumentos, com `drop function` explícito da de 16** — overload vivo faz a chamada do n8n casar com AMBAS e o Postgres recusar com `function is not unique` no meio de um lote (o incidente que a 0118 consertou). **O portão de aridade de `reextracao.test.sql` passa de 16 para 17 na mesma passada** — ele não "quebrou", ele fez o trabalho dele. **Sonda:** requisito de corpo pela CHAMADA (`p_entidade_nome, p_cnpj`), não pelo nome da função chamada — `fn_upsert_entidade` está no corpo desde a 0030 e o requisito nasceria vazio. **Invariante:** bloco 8 de `cnpj_identidade.test.sql`, que prova o fio INTEIRO pela porta (dois documentos com grafias diferentes e o mesmo CNPJ → UMA entidade, os dois documentos nela), em vez de chamar `fn_upsert_entidade` direto. **AINDA FALTA a IA LER o CNPJ** — prompt e `SCHEMA_CLASSIF` em `N8N/build-workflow.mjs`; até lá o parâmetro chega nulo e nada muda. Antes dela: `Supabase/migrations/0169_o_cnpj_e_a_identidade_que_o_nome_nao_e.sql` — **o CNPJ é a identidade que o nome não é.** A 0168 reduziu a OMNIBEAUTY de QUATRO entidades para TRÊS e parou ali por razão medida: "…DE MARCAS" e "…DE SURUBIJU" NÃO casam por `fn_mesma_entidade`, e escolher uma seria ESCOLHER NO EMPATE (o que a 0153 proíbe). Quem prova que são a mesma empresa é o **CNPJ 36.193.378/0001-04**, igual nos dois balanços anexados. A coluna `entidade.cnpj` existe desde a 0001 e **nunca foi populada em lugar nenhum do produto**. **AS TRÊS REGRAS, em ordem de força:** (1) **CNPJ igual é a mesma empresa**, sem olhar o nome — é isto que funde as variantes truncadas em QUALQUER ordem de chegada, coisa que a 0168 só conseguia quando a ordem ajudava; (2) **CNPJ diferente é outra empresa** e tira a candidata da lista (`fn_entidades_candidatas_cnpj`) — o nome não tem autoridade para contradizer o registro fiscal, e isso corrige, QUANDO HÁ CNPJ, o LIMITE CONHECIDO da 0153 (hoje "ALFA COMERCIO EXTERIOR LTDA." é absorvida em silêncio por "ALFA COMERCIO LTDA."); (3) **CNPJ ausente não decide nada** — null é ausência, não informação (regra 1). **O CNPJ é APRENDIDO** (`fn_entidade_aprender_cnpj`, com `entidade_cnpj_aprendido` no log): o primeiro documento ensina a identidade, os demais a usam; **nunca sobrescreve** CNPJ já gravado. **DV CONFERIDO** (`fn_cnpj_canonico`): o CNPJ vai chegar de uma IA lendo PDF escaneado, e um número inventado que passe como identidade funde duas empresas de verdade em silêncio — pior que não ter CNPJ nenhum; DV que não fecha é AUSÊNCIA. **A ARMADILHA DO OVERLOAD, tratada explicitamente:** acrescentar parâmetro com default sem matar a assinatura antiga deixa DUAS funções vivas e a chamada de 2 args casa com AMBAS — `function is not unique` no meio de uma rodada, que foi o que a 0118 teve de consertar em `fn_registrar_documento`; daí o `drop function fn_upsert_entidade(uuid, text)` antes do `create`. **Invariante** (`Supabase/test/cnpj_identidade.test.sql`), 17 asserts em 7 blocos: (1) os quatro nomes reais + o CNPJ real = **UMA** entidade; (2) e em qualquer ordem de chegada; (3) **CNPJ nulo não muda NADA** — os mesmos quatro nomes sem CNPJ continuam dando TRÊS, que é a propriedade de segurança desta migration; (4) CNPJ diferente separa mesmo com o nome casando, E sem CNPJ o limite da 0153 continua exatamente como era; (5) Araucária sem CNPJ continua sem escolher, com CNPJ vai para a empresa certa; (6) CNPJ inválido não funde nem é gravado; (7) o aprendizado tem rastro e não sobrescreve. **NÃO faz o CNPJ CHEGAR até aqui** — isso é a extração (prompt + schema de `N8N/lib/extract.mjs` + o nó que chama `fn_registrar_documento`), e é a fatia seguinte. Antes dela: `Supabase/migrations/0168_o_apelido_truncado_nao_e_ambiguidade.sql` — **o nome truncado que casa com os outros virava uma EMPRESA A MAIS.** No export do caso "teste 143" a OMNIBEAUTY aparece como QUATRO empresas, em quatro blocos de coluna separados (`…GESTAO DE` 7 docs, `…DE MARCAS LTDA` 2, `…DE SURUBIJU` 1, `…DE SURUBIJU, 1930` 1) — é UMA só (CNPJ 36.193.378/0001-04 nos dois balanços anexados), e "SURUBIJU, 1930" é o **ENDEREÇO**: o template do contador trunca a razão social num campo de largura fixa e imprime o endereço na linha de baixo, sem separação (conferido em `OMNIBEAUTY - FATURAMENTO 2024.pdf`). **CAUSA:** `fn_upsert_entidade` (0153), no "no empate, não escolhe" — com 2+ candidatos aproximados e nenhum exato ela cria entidade NOVA; a partir da segunda variante toda variante nova acha 2+ candidatos e vira linha própria (**efeito cumulativo**). **E ESTA MIGRATION CORRIGE UMA HIPÓTESE QUE UMA SESSÃO ANTERIOR TINHA ESCRITO:** as quatro variantes NÃO casam par a par. Medido, os seis pares: truncado×MARCAS `t`, truncado×SURUBIJU `t`, truncado×SURUBIJU,1930 `t`, SURUBIJU×SURUBIJU,1930 `t`, **MARCAS×SURUBIJU `f`**, **MARCAS×SURUBIJU,1930 `f`** ("marcas" não é prefixo de "surubiju"). Pelo NOME, "…DE MARCAS" e "…DE SURUBIJU" são tão indistinguíveis de duas irmãs quanto Bioenergia × Imobiliária — fundi-las seria ESCOLHER no empate, que é o que a 0153 proíbe. **A CORREÇÃO, a fatia que o nome sozinho prova:** `fn_entidades_sao_um_grupo(text[])` (verdadeiro quando CADA PAR casa — **não é fecho transitivo, de propósito**: A~B e B~C sem A~C é o apelido curto que casa com duas empresas distintas) e, antes de criar a entidade por ambiguidade, se os candidatos **mais o nome que chega** formam um grupo único, fica o candidato de razão social mais completa, com `entidade_alias_fundido` no log. O nome que chega entra na conta do grupo mas **não na escolha** — é ele que pode estar contaminado pelo endereço. **GANHO MEDIDO** contra os quatro nomes reais, na ordem que reproduz o export: **4 entidades → 3**. **E O QUE FALTA, dito com o número:** fechar de 3 para 1 exige o CNPJ chegar até aqui — é mudança na EXTRAÇÃO, não neste algoritmo, e está registrada no `HANDOFF.md`. **A ordem importa e está medida:** com o truncado chegando PRIMEIRO, cada variante acha UM candidato e é absorvida — uma entidade, mesmo sem a 0168; o lote real chegou na outra ordem. **Invariante** (`Supabase/test/alias_truncado.test.sql`), quatro blocos: (1) os quatro nomes reais na ordem do lote dão 3 entidades — desligada, dão **4**, as quatro linhas do export — e a variante do endereço entra na de nome mais completo com rastro; (2) a ordem favorável continua dando 1 (o ramo do candidato único não regrediu); (3) **contrapositivo da 0153**: "Araucaria SPE" contra Bioenergia × Imobiliária segue em entidade própria com `entidade_ambigua` e SEM nenhum `alias_fundido`; (4) o grupo não é fecho transitivo, isolado. **Sonda:** requisito de `corpo` sobre `fn_upsert_entidade` com marcador `fn_entidades_sao_um_grupo` — o nome da função nova, que some num `create or replace` com o corpo velho. **NÃO funde o que já está fragmentado:** o caso "teste 143" continua com as quatro linhas até o dono decidir — é dado de produção, não código. Antes dela: `Supabase/migrations/0167_o_faturamento_que_somava_a_coluna_total.sql` — **o faturamento somava a coluna "Total" JUNTO das categorias, e o painel dizia "48 meses" num ano.** Medido célula a célula no export do caso "teste 143" — e CORRIGE uma hipótese errada de uma sessão anterior (a de que era sobreposição de datas entre dois arquivos; é um documento só). O relatório real (`GENERAL TABACO - FATURAMENTO 2024.pdf`) tem 12 linhas de mês e QUATRO colunas de valor (Saídas/Serviços/Outros/**Total**), e cada célula vira uma linha de `campo_extraido` com o MÊS na `chave` e a CATEGORIA em `periodo_coluna`. `fn_somar_faturamento_ano` excluía os totais com `fn_normalizar_texto(ce.chave) not like '%total%'` — **mas "total" não está na chave, está em `periodo_coluna`**. A exclusão nunca casava: 12 meses × 4 categorias = **48 "meses"**, exatamente o número da mensagem, e a soma saía em DOBRO (Total já é Saídas+Serviços+Outros). **Conferência aritmética:** Receita Bruta da DRE 74.160.836,36 × 2 = 148.321.672,72 contra os 144.872.009,06 somados — bate em 97,7%, e o resíduo de 2,3% é a diferença de competência que a própria mensagem já chamava de "Classe B". **A CORREÇÃO:** agrupar por RÓTULO (o mês) antes de somar; dentro do mês, se existe célula cuja `periodo_coluna` diz "total", ela é a resposta — as outras são a decomposição dela. Sem coluna de total, soma-se a quebra (comportamento de hoje). **E `n_linhas` passa a contar MESES** (12, não 48): ele vira "%s meses de faturamento" na mensagem, e dizer 48 era mentira sobre a UNIDADE. **Invariante** (`Supabase/test/faturamento_por_mes.test.sql`), com a forma da tabela real e os TRÊS formatos: (1) 3 meses × 4 categorias somam 14.379.150,46 e contam 3 — desligada, somam o dobro e contam 12; (2) um valor por mês sem `periodo_coluna` (o book-vertentes) não muda; (3) quebra por categoria SEM total soma as categorias — sem esse bloco a regra poderia ter virado "só conta o total" e esse formato zeraria em silêncio. Antes dela: `Supabase/migrations/0166_a_exigencia_que_nao_conhecia_o_passivo_bare.sql` — **a MESMA convenção da 0165 derrubava o Kit Básico por um caminho DIFERENTE.** A 0165 corrigiu a reconciliação (que compara VALORES); a exigência `passivo_mais_pl` do checklist só pergunta se a linha EXISTE, e tinha quatro localizadores — todos exigindo a palavra "total" no rótulo ou o par estrutural `['passivo','patrimonio']`. **Nenhum casava "PASSIVO" sozinho**, e o lote real abriu CINCO pendências `linha_exigida_ausente` ("a linha exigida 'Passivo + Patrimônio Líquido (total)' não foi localizada"), uma por entidade + o COMBINADO, cobrando do cliente uma linha que ele já entregou. **E o seed da 0113 já previa isso por escrito**: *"se um dia a reconciliação mudar de termos, este seed é o segundo lugar a atualizar"* — a reconciliação mudou na 0034 (ganhou o fallback estrutural) e o seed ficou 52 migrations para trás. **A CORREÇÃO:** um quinto localizador, `estrutural ['passivo']`, espelhando o fallback que a reconciliação já tinha. Não afrouxa nada: o Kit Básico responde "existe alguma linha de lado direito?" e quem confere o VALOR é a A.1 da 0165, no mesmo documento. **Sonda por VIEW** (`instalacao_sonda_passivo_bare`), e isso não é detalhe: o requisito de `seed` conta a TABELA INTEIRA, então apontá-lo para `taxonomia_linha_localizador` o faria passar num banco sem a migration (dezenas de localizadores de outras exigências) — requisito nascido vazio dentro da própria sonda. A view isola as DUAS linhas inseridas. **Invariante** (bloco 4 de `passivo_bare_e_o_grupo.test.sql`): a exigência é satisfeita pelo balanço real da AMOBELEZA, e o localizador casa "PASSIVO" mas NÃO "PASSIVO CIRCULANTE" nem "PASSIVO E PATRIMÔNIO LÍQUIDO" — sem esse contrapositivo, meio balanço satisfaria o lado direito inteiro. Antes dela: `Supabase/migrations/0165_o_passivo_que_ja_inclui_o_patrimonio.sql` — **"PASSIVO" sozinho JÁ É Passivo + PL, e a checagem A.1 somava o PL duas vezes.** Medido contra o `AMOBELEZA - BALANÇO 2024.pdf` que o dono anexou (lote do caso "teste 143"): `ATIVO 118.591.566,53` e `PASSIVO 118.591.566,53` — iguais, porque a linha "PASSIVO" deste ERP é o total do LADO DIREITO (`PASSIVO CIRCULANTE 119.620.673,17 + PASSIVO NÃO-CIRCULANTE 8.362.115,49 - PATRIMÔNIO LÍQUIDO 9.391.222,13 = 118.591.566,53`), não o exigível. É convenção de plano de contas brasileiro, conferida nos TRÊS balanços com camada de texto que o dono mandou, de duas empresas e dois modelos de relatório. **O DEFEITO:** os dois primeiros localizadores do lado direito pedem "total" no rótulo ou o par estrutural `['passivo','patrimonio']` — "PASSIVO" sozinho não casa nenhum, cai no terceiro caminho (estrutural `['passivo']` + `['patrimonio']`) e SOMA os dois: `118.591.566,53 + (-9.391.222,13) = 109.200.344,40`, exatamente o "Passivo+PL" que o painel mostrou, com "diferença de 9.391.222,13" — o PL contado duas vezes. Com PL negativo a conta fica abaixo do ativo; com PL positivo, acima: as **onze** pendências falsas do lote são as duas direções do mesmo defeito. **A CORREÇÃO, estreita de propósito:** quando o passivo veio do casamento ESTRUTURAL **e já bate com o Ativo dentro da tolerância da própria checagem**, ele É o total do grupo — usa direto, sem somar o PL; o valor desambigua o rótulo, que sozinho é ambíguo. Rótulo que DIZ "Passivo Total" continua sendo exigível e somando o PL; "PASSIVO" bare que NÃO bate com o ativo também. **Invariante** (`Supabase/test/passivo_bare_e_o_grupo.test.sql`), com os números reais do PDF e medido nos TRÊS sentidos: (1) o balanço real fecha — com a correção desligada reprova em `divergente / 9391222.13`; (2) contrapositivo — "PASSIVO" bare exigível de verdade continua somando o PL (sem este bloco a correção poderia ter virado "nunca mais soma"); (3) balanço que não fecha DE VERDADE continua `divergente` — a correção não cegou a checagem. Antes dela: `Supabase/migrations/0164_a_estimativa_que_o_filtro_opaco_escondia.sql` — **`fn_linhas_para_modelagem`/`fn_conferir_modelagem` estouravam os 8s do Supabase no caso "Teste 00" (190 documentos, 7.151 linhas)** — a mesma classe de mentira silenciosa da `0101`/`0152`: consulta cancelada por timeout chega ao portal como "este caso ainda não tem linha extraída com valor", não como erro. **CAUSA MEDIDA por EXPLAIN (ANALYZE, BUFFERS):** o filtro `dv.id = fn_versao_com_extracao(d.id)` (0102, "só a versão vigente") compara contra o retorno de uma FUNÇÃO — o planner não sabe estimar a seletividade, chuta `rows=1` para a CTE `bruto` (real: 7.220 num fixture de escala equivalente), e escolhe **Nested Loop** para o join que deveria ser Hash Join. Reproduzido num fixture sintético de 190 documentos/800-1.000+ rótulos distintos (o teto de 250 do fixture antigo mascarava o defeito): **15,0s medidos, fresco, sem ANALYZE manual** — o estado real de um caso recém-carregado. **A CORREÇÃO:** a "versão vigente" vira uma CTE nomeada (`versao_vigente`, `distinct on` + `order by n_versao desc`, mesma regra da 0102) — um JOIN que o planner consegue ver e estimar, em vez de um filtro opaco por linha. **MEDIDO O DEPOIS**, mesmo caso: Nested Loop → Hash Join, `rows=1` → `rows=6434` (real 7.220), **15,0s → 2,6s (5,7×)**, com folga sob o teto de 8s. **RESULTADO IDÊNTICO**, provado por `except` nos dois sentidos contra os 75 casos do `run.sh` + 3 fixtures sintéticos: uma divergência de sinal por empate de módulo não-determinístico apareceu na primeira rodada (`dmpl / Reserva de lucros a realizar`, fixture Canastra — `valor_ultimo` `6834.0` × `-6834.0`, ambiguidade LATENTE do corpo anterior, exposta pela mudança de plano, não causada por ela) — corrigida com desempate declarado (`, o.valor_num desc`) em `valor_ultimo`; segunda rodada, **0 divergências em 78 casos**. **Teste de escala novo** (`Supabase/test/modelagem_versao_vigente_escala.test.sql`, 190 docs/~7.200 ocorrências/1.000 rótulos distintos): com a correção desligada, reprova por `canceling statement due to statement timeout` (o mesmo sintoma real do Supabase); religada, passa em ~10s de fixture + <3s de chamada. **Sonda:** `corpo` (`versao_vigente`, identificador estrutural, não comentário decorativo) + `comportamento` executado (fixture permanente "Sonda 0164", dois documentos multi-versão: reextração que corrige valor, reextração em andamento sem extração ainda) — não repete o padrão de 19 requisitos anteriores cegos a marcador de comentário/número de migration. **Não mexe no self-join de sobreposição em `pares`** (CTE de `base_r`, hipótese descartada como secundária no diagnóstico original) — cresce com o nº de rótulos e já custa ~7,4s sozinho a 2.000 rótulos: fora do escopo desta migration, registrado para quem for medir a próxima vez que o caso real crescer mais. Antes dela: `Supabase/migrations/0163_a_ancora_que_o_crlf_desalinhou.sql` — **o dono tentou aplicar a `0160`/`0161`/`0162` em produção: a `0160` já estava aplicada (reexecução recusada, CORRETO), e a `0161`/`0162` ABORTARAM SEM ESCREVER NADA** — `0161`: *"o bloco do format() de periodo_incorreto não foi encontrado UMA vez (achei 0)"*; `0162`: *"o patch apagou o da 0161 — abortado"* (consequência direta de a 0161 não ter entrado). `fn_instalacao_conferir()` em produção devolve **ZERO linhas**. **A CAUSA É CRLF**: o corpo de `fn_registrar_diagnostico` em produção está gravado com `\r\n` (sonda do dono: `bytes=13620`, contra `13.414` do estado 0160 local — 206 bytes/linhas a mais), e a `0161`/`0162` nunca tiveram `\r?\n` em âncora nenhuma (ao contrário da `0142`/`0143`/`0145`/`0160`, que já sabiam). **REPRODUZIDO NESTA ÁRVORE**: o corpo do banco de teste `tdf_0160` convertido para CRLF (13.631 bytes) e a `0161` rodada sobre ele falha com a MESMA mensagem, literal, do dono. A `0163` para de patchar por âncora e **reemite `fn_registrar_diagnostico` INTEIRA** com o corpo VERBATIM do estado `0162` (fonte: `pg_get_functiondef` do banco `tdf_0162`, idêntico char a char ao que `Supabase/schema.sql` já publicava) — seguro porque a `0162` é a migration mais nova do repositório, não há patch mais novo para regredir em silêncio. Garante também `fn_entidade_e_balcao_ambiguo` (0162) e reinstala, IDEMPOTENTE, os quatro requisitos de catálogo da `0161`/`0162` que nunca chegaram ao banco de produção (o `insert` delas morava depois do patch que abortou), a view/fixture "Sonda 0162", e guarda o RESULTADO da reemissão (os quatro marcadores + a assinatura, normalizados contra CRLF com `replace(chr(13),'')`) em vez de confiar. **PROVADO EM DOIS ESTADOS**: (a) aplicada sobre `tdf_0160` (CRLF, sem `0161`/`0162`) — `md5(replace(pg_get_functiondef(...), chr(13), ''))` deu **`8db263c25bd6d7199145b1999e8cd320`**, IGUAL ao mesmo hash tirado de `tdf_0162`, e o corpo ficou LF puro (`position(chr(13) in ...) = 0`); (b) nesta árvore, onde `0161`/`0162` já tinham aplicado, a `0163` roda por último como NO-OP semântico e `Supabase/schema.sql` regenerado continua idêntico ao commitado. **NÃO mexe em pendência nenhuma já aberta** — instalação de código, não correção de dado; o efeito só aparece na próxima vez que `fn_registrar_diagnostico` rodar sobre cada documento. **Guarda de repositório nova** (`Supabase/test/guarda_ancora_crlf.py`, ligada no `run.sh`): varre `Supabase/migrations/*.sql` e reprova qualquer migration futura que patcheie corpo de função com `regexp_matches`/`regexp_replace` sobre um padrão MULTI-LINHA sem `\r?` — medido: reprova (2 violações) contra uma cópia da `0160` com todo `\r?` removido, e passa contra o diretório real (107 migrations, nenhuma violação). Memória nova: `.claude/memory/ancora-de-texto-quebra-com-crlf.md`. Antes dela: `Supabase/migrations/0162_o_balcao_que_nao_escuta_a_resposta.sql` — **o balcão de perguntas da `0153` respondia por ele mesmo, e a resposta do PRÓPRIO documento era jogada fora.** Reproduzido nesta árvore (banco `tdf_test`), padrão do lote `7417` (araucária, 03/09): o documento `009` fica no balcão "Araucaria SPE" (pendência `entidade_ambigua` bloqueante contra BIOENERGIA × IMOBILIÁRIA), e o diagnóstico de conteúdo do PRÓPRIO documento nomeia "ARAUCÁRIA IMOBILIÁRIA SPE LTDA." por extenso — `fn_mesma_entidade` confirma contra o balcão (ele casa com as DUAS empresas do grupo, por construção: foi criado exatamente porque o nome dele casava com mais de uma), e `fn_registrar_diagnostico` tomava isso como CONFIRMAÇÃO, sem tocar a pendência bloqueante e sem a resposta deixar rastro em lugar nenhum. A `0162` acrescenta `fn_entidade_e_balcao_ambiguo(caso_id, entidade_id)` (verdadeiro com pendência `entidade_ambigua:<id>` ainda aberta) e, antes do `if fn_mesma_entidade(...)` vigente, confere se a entidade registrada é esse balcão — se for, o casamento NÃO confirma nada, e as candidatas EXATAS do nome diagnosticado são recalculadas (reaproveitando `fn_entidades_candidatas` da `0153`, excluído o próprio balcão): com EXATAMENTE UMA, abre pendência POR DOCUMENTO (`entidade_incorreta`/`importante`/`sobrepujavel=true`) nomeando a empresa; com zero ou duas-ou-mais, não abre nada (regra 1 do CLAUDE.md — ausência não vira dado). **NÃO move o documento, não funde entidades, não toca a pendência bloqueante da `0153`, não aperta `fn_mesma_entidade`.** Medição: **1 assert reprova** com a correção desligada — a pendência que deveria nomear a empresa não existe — e é exatamente o novo; a pendência bloqueante da `0153` e o Portão 2 fechado continuam intactos nos dois lados (com e sem a correção), provando que a correção não afrouxou a salvaguarda. **O que NÃO dá para provar:** não há o dado do lote `7417` — não se sabe a string exata que o diagnóstico de conteúdo do documento `009` devolveu; o teste reproduz o PADRÃO medido (nome de arquivo ambíguo + conteúdo que nomeia a empresa por extenso), com as duas razões sociais que a própria `0153` já mede como reais, não os números daquela chamada específica. Antes dela: `Supabase/migrations/0161_a_justificativa_que_o_periodo_perdia.sql` — **a pendência de período escondia o "por quê" que o próprio diagnóstico já tinha escrito.** Das quatro pendências do lote `7377` que sobraram sem correção (2 `tipo_incorreto`, 2 `periodo_incorreto`), a medição concluiu que **três não precisam de nada** e só uma tinha lacuna — e esse "não precisa" é resultado, não omissão. Medido no corpo VIGENTE de `fn_registrar_diagnostico`: o ramo de `tipo_incorreto` já monta `... (documento está registrado como "%s"). %s` com `coalesce(p_justificativa, '')` no fim, então o revisor dos documentos `27` e `35` já lê a razão do modelo junto da divergência; **o ramo de `periodo_incorreto` não passava `p_justificativa` para o `format()`** — o mesmo parâmetro, na mesma chamada, calculado pelo mesmo diagnóstico, perdido por assimetria de código e não por doutrina. A `0161` acrescenta só isso, por patch de âncora sobre o corpo vigente (razão da `0160`: reemitir copiando texto de migration antiga regride em silêncio um patch mais novo). **Não decide se o período certo é o intervalo ou a data-base** — continua sendo divergência real entre nome e conteúdo, que é pergunta para humano; só para de esconder o porquê. `tipo`/`severidade`/`sobrepujavel` intocados, que são os três campos (não o `motivo`) que governam a fila de revisão e o fechamento por `fn_revisar_documento`. **E ficou registrado o que NÃO dá para enriquecer:** a `confianca` do diagnóstico nunca chega ao banco — a assinatura de `fn_registrar_diagnostico` tem 11 parâmetros e nenhum é confiança, o `evento_auditoria` não a carrega, e a query do n8n não a envia; exibi-la exigiria mudar assinatura e wiring, não é "dado que o banco já sabe e não está dizendo". Medição: **1 assert reprova** com a migration removida do diretório, e é exatamente o novo. Antes dela: `Supabase/migrations/0160_a_hierarquia_que_o_diagnostico_chama_de_erro.sql` — **holding registrada, subsidiária no conteúdo: não é `entidade_incorreta`.** Medido no mesmo lote `7377` da sessão 79: `33_Notas_Explicativas` registrado em **GRUPO CANASTRA**, diagnóstico de conteúdo aponta **Canastra Indústria** — as duas já são entidades REAIS e distintas do mandato (o fixture do book Canastra as cadastra em linhas separadas), e `fn_registrar_diagnostico` abria `entidade_incorreta` como se o registro estivesse errado. **A `0153` NÃO cobre este caso** — `fn_mesma_entidade('GRUPO CANASTRA', 'CANASTRA INDÚSTRIA DE EMBALAGENS LTDA.') = false`, então não é a ambiguidade de nome que casa com DUAS entidades ao ser procurado (`fn_upsert_entidade`); é função diferente (`fn_registrar_diagnostico`, que compara entidade JÁ registrada contra a diagnosticada) e os dois nomes não casam entre si. A correção reaproveita `fn_entidades_candidatas` (0153): antes de abrir `entidade_incorreta`, confere se o nome diagnosticado casa com OUTRA(S) empresa(s) já cadastrada(s) no MESMO caso — se casar, a pendência clássica resolve e nasce uma distinta pelo `motivo` (`diagnostico:entidade_grupo:`). Divergência de verdade (nome que não bate com ninguém do caso) continua abrindo `entidade_incorreta` como na `0121`. **A REVISÃO DERRUBOU DUAS COISAS DA PRIMEIRA VERSÃO, e as duas eram do tipo que este projeto persegue.** (a) A mensagem afirmava *"Isto não é erro de registro: são duas empresas distintas do mesmo grupo (holding × subsidiária)"* — **a função não sabe disso.** Num documento que está MESMO mal registrado, o chip diz "entidade pode estar incorreta" e o corpo diz o contrário; diante da contradição o analista segue a frase longa e explicada, e o documento fica na empresa errada. `papel_no_grupo` é o único campo que registraria a relação e a função não o consulta — os três fixtures que o populam usam `'alvo'` para toda entidade, então consultá-lo também não salvaria a afirmação. A mensagem passa a dizer só o provável: as candidatas já são empresas do mandato e **o sistema não sabe qual é a certa**. (b) O `order by ... limit 1` **escolhia a primeira candidata por ordem alfabética** e sumia com as demais na mensagem — exatamente o dano que a `0153` existe para evitar, refeito na função vizinha; agora conta todas e nomeia todas. **E a contagem de asserts da primeira versão (5) estava inflada:** `teste_assert` levanta exceção, então a suíte PARA no primeiro que reprova e "N asserts reprovaram" não é legível de uma rodada só — os 5 eram uma causa raiz única contada cinco vezes. Medido com o patch da função desligado (mantendo o catálogo da sonda, senão o portão do `run.sh` aborta antes dos testes): a suíte morre no comportamento central, a pendência clássica ainda abre. Testes: seções 16–18 de `Supabase/test/canastra.test.sql`, contra o fixture real (nenhuma entidade inventada); a 18 é nova e cobre o candidato múltiplo e o caminho de TRANSIÇÃO (pendência clássica já aberta que fecha), que é o estado real do lote `7377` e não tinha assert nenhum. Antes dela: `Supabase/migrations/0159_o_rotulo_que_a_propria_estrutura_desmente.sql` — **o rótulo que o próprio sistema já contesta parou de decidir sozinho.** Medido no lote `7417` (araucária, 03/09): seis documentos — o conjunto completo de demonstrações da Serraria — saíram `COMBINADO` pela IA com **zero empresas nas colunas**, e o diagnóstico abriu `tipo_incorreto` nos seis dizendo *"trata-se de balanço... não combinado"*. Pela escala da `0151` isso vale 35 contra os 55 do balanço avulso da mesma empresa — que nesta rodada extraiu **49%** e abriu `extracao_falhou`. **Num conflito, o mal extraído vencia, em silêncio.** A `0155` não pega este lado: ela só `least()`, e o COMBINADO já é o piso — não havia o que abaixar. **O espelho estrutural foi TENTADO e MEDIDO COMO FALSO:** escrito, rodado, derrubou o `desempate.test.sql`, porque "≥2 empresas prova derivado" é prova e "<2 empresas prova NÃO derivado" não é — a ausência de `entidade_coluna` só diz que o documento não marcou. O sinal usado é o que o sistema JÁ publica: `fn_documento_decide_sozinho` devolve falso quando o documento é COMBINADO e tem `tipo_incorreto` aberta, e `fn_conflitos_do_caso` passa a exigir isso dos dois lados. **A autoridade numérica não muda** — é a disciplina do EMPATE da `0151`: o valor fica, o conflito para de ser resolvido calado e o critério diz "SEM DECISÃO AUTOMÁTICA". Resolvida a pendência, o documento volta a decidir sozinho pelo ciclo de vida dela. O teto da `0155` está intacto, conferido linha a linha. Antes dela: `Supabase/migrations/0158_o_pronto_que_nao_provava_cobertura.sql` — **a tela dizia "pronto para exportar" em VERDE sobre um modelo que não projeta.** Medido no fixture Vertentes: `pronto: true` com `linhas_com_premissa: 23` de `linhas_do_caso: 480`. O `pronto` da `0134` já exigia parâmetros + premissa ativa + nenhuma premissa sem valor — o que ele NUNCA perguntou é se alguma linha do caso chegou a ser VINCULADA. Caso com premissas boas e vínculo nenhum (ou só órfão) respondia `pronto`. A `0158` acrescenta UMA cláusula (`linhas_com_premissa > 0`) e a isola em `fn_modelagem_esta_pronta`, função PURA, para a sonda poder EXECUTÁ-LA por literais em vez de procurar marcador de texto — é o achado D da revisão da `0157` virando padrão. **Duas condições que a sessão propôs foram DERRUBADAS com evidência:** bloquear por `sazonalidade_sem_curva` reverteria a `0134` e quebraria `sazonalidade_pronto.test.sql`; bloquear por `vinculos_orfaos` quebraria `premissas.test.sql` — nenhum dos dois deixa número errado no book, só a fração pior. E a função ganhou a FRAÇÃO de contas com premissa, porque um booleano não diz "16% ficaram de fora". O denominador é `linhas_do_caso`, que apesar do nome já conta só `papel = conta` desde a `0101` — conferido no corpo, não no nome. Antes dela: `Supabase/migrations/0157_o_combinado_que_travava_o_kit_basico.sql` — **o Kit Básico parou de perguntar ao rótulo, e a revisão dela achou mais coisa do que a correção original.** O defeito de origem, medido no lote `7377`: o item obrigatório `COMBINADO` abriu como `item_faltante` BLOQUEANTE e `sobrepujavel = false` — travando o mandato sem botão de destrave no portal — enquanto DOIS documentos combinados estavam no caso, rotulados `BALANCO` pela IA com 8 empresas nas colunas cada, e a mesma rodada abria sozinha pendência `tipo_incorreto` dizendo que o diagnóstico sugere COMBINADO. É a ambiguidade que a `0155` mediu no araucária chegando pelo outro lado: a `0155` corrigiu a AUTORIDADE por critério estrutural, e o CHECKLIST continuou acreditando no rótulo. **Mas a primeira versão desta migration trocava um bloqueio honesto por um book vazio em silêncio**, e foi a revisão que pegou: contar empresas nas colunas SEM exigir número faria um combinado escaneado de que só saíram os cabeçalhos satisfazer o item, com o caso passando o Portão 1 e a parte combinada saindo vazia. A versão que ficou tem quatro peças: (a) `fn_documento_serve_como` conta só colunas com `valor_num` não-nulo; (b) `fn_combinado_estrutural_apto` restringe o tipo-FONTE a demonstração primária (`DRE`/`BALANCO`/`FLUXO_CAIXA`) — **`DF_AUDITADA` fica de fora e é PERGUNTA ABERTA PARA O DONO**, porque incluí-la trocaria "o classificador errou o rótulo" por "a auditada consolidada passa a bastar", em todo mandato já aberto; (c) `fn_exigencias_do_caso` reemitida para enxergar o tipo SERVIDO, senão as 3 exigências de linha do COMBINADO ficavam mudas em vez de avaliadas; (d) a sonda passou a EXERCITAR o predicado (view `instalacao_sonda_combinado_estrutural`, requisito `seed`) em vez de procurar um marcador de texto — medido: com a função morta, ela cai para `presente = f, 0 linha(s)`, enquanto os requisitos de `corpo`/`funcao` continuavam dizendo `t`. **`fn_recomputar_completude` e `fn_exigencias_do_caso` reemitidas inteiras, e provei por diff que as reemissões são ADITIVAS**: as únicas linhas removidas são as que deviam mudar. O portal parou de rederivar a mesma regra no cliente (`kit-basico.ts`) — sem isso, o Portão 1 abriria e a grade continuaria desenhando COMBINADO em cinza. Antes dela: `Supabase/migrations/0156_o_lote_existe_antes_de_terminar.sql` — **a linha de `lote_execucao` nasce quando o orçamento aceita o lote, não quando a cadeia termina.** A rodada de 190 de 27/08 foi cancelada e a tabela ficou vazia, levando junto `documentos_fatiados` — o número de que a investigação da sub-extração precisava. `fechado_em` nulo passa a ser a informação: começou e não terminou, que antes era indistinguível de nunca ter rodado. Antes dela: `Supabase/migrations/0155_combinado_se_reconhece_pela_estrutura.sql` — **um documento com quinze empresas nas colunas é um combinado**, e o catálogo não precisa acreditar no nome dele. Medido no araucária: `053`/`054`/`055`/`057` saíram **BALANCO** e o `056`, com o mesmo padrão de nome, **COMBINADO** — todos pela IA com confiança 1,0, todos com 14–15 empresas nas colunas. Chamado de BALANCO, o combinado sobe de 30 para 50 e **empata** com o balanço individual — e empate, pela `0151`, mantém o de maior módulo: a armadilha central do araucária voltando pela porta da classificação. O critério passa a ser estrutural, como a `0146` faz do outro lado. Antes dela: **`0154`** (a pendência de cobertura diz a unidade — pares conta×coluna contra linhas do documento), **`0153`** (o nome que casa com duas empresas não identifica nenhuma) e **`0152`** (a reconciliação do lote, cada checagem sobre a chave dela: 247 invocações contra ~8.500, e `fn_conflitos_do_caso` de **12.357 ms para 1.790 ms**, medido em produção). |
| **Aplicadas no Supabase** | **até a `0157`, aplicada pelo dono em 03/09** (sonda voltou zero linhas, e o `fn_recomputar_completude` do "teste Canastra" derrubou a bloqueante do COMBINADO: 1 → 0). **A `0160` TAMBÉM JÁ ESTÁ APLICADA** — o dono tentou reaplicá-la em 10/09 e o próprio portão da migration recusou por já a encontrar no corpo (CORRETO: reexecução, não regressão). **SEIS estão NO REPOSITÓRIO e NÃO foram aplicadas: a `0158`, a `0159`, a `0161`, a `0162`, a `0163` e a `0164`** (a `0160` sai desta lista porque JÁ ESTÁ aplicada — ver acima; a `0161`/`0162` tiveram a aplicação TENTADA em 10/09 e ABORTARAM SEM ESCREVER NADA, por causa do corpo em CRLF — ver a linha "Última migration" para a causa medida e a `0163`, que resolve isto reemitindo a função inteira: **quem acrescenta migration atualiza a CONTAGEM aqui na mesma passada**, senão o número vira decoração igual ao cabeçalho do handoff). Enquanto a `0158` não for aplicada, a pendência bloqueante do lote `7377` continua travando aquele mandato; depois de aplicada ela se resolve sozinha no próximo `fn_recomputar_completude` do caso. **A `0163`, uma vez aplicada, NÃO se resolve sozinha** — igual à `0160`/`0161`/`0162`: reemitir a função não reprocessa documento nenhum; o efeito só aparece na próxima vez que `fn_registrar_diagnostico` rodar sobre cada documento. Nenhuma linha deste arquivo é autoridade sobre isso: quem responde é `fn_instalacao_conferir()` contra o banco em que se está conectado. Sobre as anteriores: **até a `0156` — as seis pendentes foram APLICADAS em 02/09**, e o `workflow.e1-ingestao.json` foi **reimportado** no n8n. Conferido pelo DONO, contra o banco de produção, com a consulta por objeto da sessão 78: as `0151`–`0156` voltaram `APLICADA` com a coluna de faltantes vazia, e o veredito das três chamadas do workflow voltou **`PODE RODAR`** — inclusive a assinatura nova de `fn_reconciliar_por_documento` (`p_escopo`, da 0152), que é a que um teste por nome daria como presente estando ausente. **Os dois passos manuais que bloqueavam a rodada caíram.** Esta linha registra medição do dono, não minha: a sessão não tem conexão com produção, e a conferência que vale é sempre a que se roda contra o banco em que se está conectado — `Supabase/test/conferir-chamadas.mjs` ou a consulta de catálogo. |
