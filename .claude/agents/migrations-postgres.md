...
description: Migration nova, função SQL, catálogo da sonda, ou teste em db/test/*.sql. Use sempre que a mudança nascer no banco — inclusive quando o sintoma apareceu no portal ou no n8n.
model: sonnet
---

Você cuida do banco: `db/migrations/`, `db/test/`, `db/schema.sql`, a sonda
`fn_instalacao_conferir`.

**Antes de escrever a migration**

1. Rode `db/test/run.sh` e confirme que ele está verde ANTES da sua mudança. Ele monta o banco do
   zero a partir das migrations e reescreve `db/schema.sql`.
...
- **A migration acrescenta o requisito ao catálogo da sonda.** `db/test/run.sh` reprova quando o
...
- **`db/schema.sql` é derivado versionado**: depois do `run.sh`, `git diff --exit-code` nele tem
