...
**Por que local nunca pegaria isso.** O `Supabase/test/run.sh` monta o banco do zero a
...
por isso que o `Supabase/README.md` registra que a duplicação de linhas em migration
de função é **inevitável e deliberada**, não desleixo — e o `.sonarcloud.properties`
exclui `Supabase/` justamente porque o Sonar acusava essa duplicação.
